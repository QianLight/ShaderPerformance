...
namespace UPOLuaProfiler
{
    ...
    public class LuaHook
    {
        ...
        public static void HookRef(IntPtr L, int reference, LuaDLL.tolua_getref_fun refFun = null)
        {
            if (isHook)
            {
                LuaLib.DoRefLuaFun(L, "lua_upo_add_ref_fun_info", reference, refFun);
            }
        }
        ...
        public static void HookUnRef(IntPtr L, int reference, LuaDLL.tolua_getref_fun refFun = null)
        {
            if (isHook)
            {
                LuaLib.DoRefLuaFun(L, "lua_upo_remove_ref_fun_info", reference, refFun);
            }
        }
        ...
        public static LuaDiffInfo RecordStatic()
        {
            ...
            LuaDLL.lua_getglobal(L, "upo_handle_error");

            LuaDLL.lua_getglobal(L, "upo_do_record");
        ...
        private static void SetTable(int refIndex, Dictionary<LuaTypes, HashSet<string>> dict,
            Dictionary<string, List<string>> detailDict)
        {
            IntPtr L = LuaProfiler.mainL;
            if (L == IntPtr.Zero)
            {
        ...
        {
            LuaNetworkService.SendMessage(Diff());
        }

        public static void MarkRecordServer()
        {
            LuaNetworkService.SendMessage(Record());
        }
        ...
        public static LuaDiffInfo Diff()
        {
            ...
            LuaDLL.lua_getglobal(L, "upo_diff");
            LuaDLL.lua_getref(L, historyRef);
            LuaDLL.lua_getref(L, staticHistoryRef);
            if (LuaDLL.lua_type(L, -1) != LuaTypes.LUA_TTABLE &&
        ...
        public static void RefString(IntPtr strPoint, int index, object s, IntPtr L)
        {
            int oldTop = LuaDLL.lua_gettop(L);
            //把字符串ref了之后就不GC了
            LuaDLL.lua_getglobal(L, "UPOLuaProfilerStrTb");
    ...
    public class UPOLuaProfilerLuaProfilerWrap
    {
        ...
        public static void __Register(IntPtr L)
        {
            ...
            LuaDLL.lua_rawset(L, -3);
            LuaDLL.lua_setglobal(L, "UPOLuaProfiler");

            LuaDLL.lua_pushstdcallcfunction(L, unpackReturnValue);
            LuaDLL.lua_setglobal(L, "upo_unpack_return_value");

            LuaDLL.lua_pushstdcallcfunction(L, addRefFunInfo);
            LuaDLL.lua_setglobal(L, "upo_add_ref_fun_info");

            LuaDLL.lua_pushstdcallcfunction(L, removeRefFunInfo);
            LuaDLL.lua_setglobal(L, "upo_remove_ref_fun_info");

            LuaDLL.lua_pushstdcallcfunction(L, checkType);
            LuaDLL.lua_setglobal(L, "upo_check_type");

            LuaDLL.lua_pushstdcallcfunction(L, handleError);
            LuaDLL.lua_setglobal(L, "upo_handle_error");

            LuaDLL.lua_newtable(L);
            LuaDLL.lua_setglobal(L, "UPOLuaProfilerStrTb");
...
local function get_table_info(tb)
    ...
    if not result then
        ...
        if metaTb and upo_check_type(metaTb) == 2 and rawget(metaTb, '__tostring') then
...
function lua_upo_add_ref_fun_info(data)
    local result = ''
    local addr = ''
    local t = 1
    local typeStr = upo_check_type(data)
    if typeStr == 1 then
        result,addr = upo_get_fun_info(data)
        t = 1
    elseif typeStr == 2 then
        result,addr = get_table_info(data)
        t = 2
    end
    upo_add_ref_fun_info(result, addr, t)
end

function lua_upo_remove_ref_fun_info(data)
    ...
    upo_remove_ref_fun_info(result, addr, t)
end
";

        const string null_script = @"
function upo_is_null(val)
...
local infoTb = {}
local cache_key = 'upo_record_prefix_cache'

function upo_do_record(val, prefix, key, record, history, null_list, staticRecord)
    ...
    if val == upo_do_record then
        return
    end
    if val == upo_diff then
        return
    end
    if val == lua_upo_remove_ref_fun_info then
        return
    end
    if val == lua_upo_add_ref_fun_info then
        return
    end
    if val == history then
        return
    ...
    if val == upo_get_fun_info then
        return
    end
    if val == UPOLuaProfilerStrTb then
        return
	end
    ...
    if null_list then
        if type(val) == 'userdata' then
            local st,ret = pcall(upo_is_null, val)
            if st and ret then
    ...
    if history == nil then
        ...
        if record_val == nil then
            ...
            if typeStr == 'function' then
                local funInfo = upo_get_fun_info(val)
    ...
    if typeStr == 'table' then
        for k,v in pairs(val) do
            ...
            if typeKStr == 'table' or typeKStr == 'userdata' or typeKStr == 'function' then
                key = 'table:'
                upo_do_record(k, tmp_prefix, 'table:', record, history, null_list, staticRecord)
            end
            upo_do_record(v, tmp_prefix, key, record, history, null_list, staticRecord)
        end

    elseif typeStr == 'function' then
        if val ~= lua_upo_add_ref_fun_info and val ~= lua_upo_remove_ref_fun_info then
            local i = 1
            while true do
                ...
                if v then
                    local funPrefix = upo_get_fun_info(val)
                    upo_do_record(v, funPrefix, k, record, history, null_list, staticRecord)
                end
                i = i + 1
            end
        end
    ...
    local metaTable = getmetatable(val)
    if metaTable then
        upo_do_record(metaTable, tmp_prefix, 'metaTable', record, history, null_list, staticRecord)
    end
end

-- staticRecord为打开UI前的快照， record为打开UI后的快照，add为关闭并释放UI后的快照
function upo_diff(record, staticRecord)
    local add = { }
    setmetatable(add, weak_meta_key_table)
    local null_list = { }
    setmetatable(null_list, weak_meta_key_table)
    upo_do_record(_G, '', '_G', add, record, null_list, staticRecord)
    upo_do_record(debug.getregistry(), '', '_R', add, record, null_list, staticRecord)
    local remain = { }

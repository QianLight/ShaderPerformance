...
namespace UPOLuaProfiler
{
    public static class Parse
    {
        public static readonly string LOCAL_PROFILER =
            "local BeginUPOSample = UPOLuaProfiler.LuaProfiler.BeginSample "
            + "local EndUPOSample = UPOLuaProfiler.LuaProfiler.EndSample " +
            "local upo_unpack_return_value = upo_unpack_return_value ";
        ...
        public static string InsertSample(string value, string name)
        {
            ...
            string sampleStr = string.Format("{0}BeginUPOSample(\"[lua]:require {1},{1}&line:1\")", LOCAL_PROFILER,
                name);
                    ...
                    case (int) TK.FUNCTION:
                        ...
                        while (tokenType != (int) TK.EOS)
                        {
                            ...
                            if (tokenType == (int) ')')
                            {
                                ...
                                string profilerStr = string.Format(" BeginUPOSample(\"[lua]:{0},{1}&line:{2}\") ",
                                    funName, l.Source, l.LineNumber);
                    ...
                    case (int) TK.RETURN:
                        ...
                        while (tokenType != (int) TK.EOS)
                        {
                                ...
                                || tokenType == (int) TK.EOS)
                            {
                                ...
                                returnStr = " return upo_unpack_return_value(" +
                                            returnStr.Substring(6, returnStr.Length - 6) + ") ";
                    ...
                    case (int) TK.END:
                        if (tokens.Count > 0)
                        {
                            ...
                            if (token == (int) TK.FUNCTION)
                            {
                                if (!hasReturn)
                                {
                                    l.InsertString(lastPos - 1, " EndUPOSample() ");
                                    lastPos = nextPos;
                                    nextPos = l.pos;
                                }

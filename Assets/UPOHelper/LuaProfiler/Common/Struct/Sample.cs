...
namespace UPOLuaProfiler
{
    ...
    public class LuaRefInfo : NetBase
    {
        ...
        public override byte[] Serialize()
        {
            ...
            buf = null;
            buf = Encoding.UTF8.GetBytes(addr);
            bw.Write(buf.Length);
            bw.Write(buf);
    ...
    public class Sample : NetBase
    {
        ...
        private static Dictionary<object, Dictionary<object, string>> m_fullNamePool =
            new Dictionary<object, Dictionary<object, string>>();
        ...
        public static void SerializeList(List<Sample> samples, string path)
        {
            ...
            for (int i = 0, imax = samples.Count; i < imax; i++)
            {
                Sample s = samples[i];
#if UNITY_EDITOR
                UnityEditor.EditorUtility.DisplayProgressBar("serialize profiler data", "serialize " + s.name,
                    (float) i / (float) imax);
#endif
        ...
        public override byte[] Serialize()
        {
            ...
            b.Write((ushort)childs.Count);
            for (int i = 0, imax = childs.Count; i < imax; i++)
            {
                byte[] buf = childs[i].Serialize();
                b.Write(buf);
            }

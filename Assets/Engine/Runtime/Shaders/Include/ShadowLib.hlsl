...
FShadowData ShadowCompareParallel(inout FFragData FragData,
...
#ifdef _CSM3
	FLOAT3 shadowCoords[4] = FragData.ShadowCoord;
#else//!_CSM3
	FLOAT3 shadowCoords[3] = FragData.ShadowCoord;
#endif//_CSM3
	FLOAT shadow = 1;
	FLOAT dynamicShadow = 1;
	FLOAT shadowMapIndex = 0;
//	_shadowMapIndex=2;
	FLOAT4 worldPos = FragData.WorldPosition;

	#if !defined(_NO_CSM)&&!defined(_SIMPLE_SHADOW)
		FLOAT2 delta0 = CalcDistDelta(worldPos + _ShadowMoveOffset,_ShadowMapParam0[0]);
		FLOAT2 delta1 = CalcDistDelta(worldPos + _ShadowMoveOffset,_ShadowMapParam0[2]);
#ifdef _CSM3
		FLOAT2 delta2 = CalcDistDelta(worldPos + _ShadowMoveOffset,_ShadowMapParam1[0]);
	   
	     delta2=_PlayerShadowOnly?0:1;
		 
#else//!_CSM3
		FLOAT2 delta2 = 0;	
#endif//_CSM3
	
		DEBUG_CUSTOMDATA_PARAM(ShadowMapIndex, 3)
		...
		if(weights.z>0)
		{
			...
			FLOAT3 shadowMapIndexMap = FLOAT3(0,1,2);
			shadowMapIndex = dot(weights.xyz,shadowMapIndexMap);
			//shadowMapIndex = 0;
#ifndef _CSM3
			shadowMapIndex = saturate(shadowMapIndex);
#endif//_CSM3
          //   _shadowMapIndex=shadowMapIndex;//TEST  blend csm and  shadowmask
			// if(shadowMapIndex<1){				
			DEBUG_CUSTOMDATA_PARAM(ShadowMapIndex, shadowMapIndex)
			FLOAT3 shadowCoord = shadowCoords[(uint)shadowMapIndex];

			#ifdef _USE_CUSTOM_SHADOW_BIAS
				shadowCoord.z += _CUSTOM_SHADOW_BIAS;
			#endif//_USE_CUSTOM_SHADOW_BIAS

			shadow = SampleShadowMap(TEX2D_PARAM(_ShadowMapTex),shadowCoord, shadowMapIndex);
		//	}
			shadow *= mask;
			FLOAT fade = dot(weights.xyz,_ShadowMapFade.xyz);
			shadow = lerp(1,shadow,fade);

			//shadow = saturate(shadow + _ShadowBias);
		}
		DEBUG_CUSTOMDATA_PARAM(SceneShadow, shadow)
	#endif//_NO_CSM
	
	#ifdef _SIMPLE_SHADOW
		//FragData.ShadowCoord2.z += _ChunkShadowBias;
		//REAL simpleShadow = SampleShadowMap2(TEX2D_PARAM(_CustomShadowMap), FragData.ShadowCoord2);
		////shadow = (simpleShadow.x < 0 || simpleShadow.x>1 || simpleShadow.y < 0 || simpleShadow.y>1) ? 1 : extraShadow;
		//DEBUG_CUSTOMDATA_PARAM(SimpleShadow, simpleShadow)
		//shadow *= simpleShadow;
		...
		//extra shadow
		UNITY_BRANCH
		if(_ExtraShadowEnable)
		{	
			#ifdef _CSM3
				FLOAT3 extraShadowCoord = shadowCoords[3];
			#else//!_CSM3
				FLOAT3 extraShadowCoord = shadowCoords[2];
			#endif//_CSM3
			extraShadowCoord.z = max(extraShadowCoord.z,0.0001f);
			FLOAT extraShadow = SampleShadowMap2(TEX2D_PARAM(_ShadowMapExtraTex),extraShadowCoord);
			extraShadow = (extraShadowCoord.x<0||extraShadowCoord.x>1||extraShadowCoord.y<0||extraShadowCoord.y>1)?1:extraShadow;
			DEBUG_CUSTOMDATA_PARAM(ExtraShadow, extraShadow)	
			dynamicShadow *= extraShadow;
			shadow *= extraShadow;
		}

		#ifndef _SIMPLE_EXTRA_SHADOW
		UNITY_BRANCH
		if(_Extra2Enable)
		{	
			FLOAT3 extra1ShadowCoord = GetShadowCoord(worldPos,MaterialData.WorldNormal, _ObjectBias,_ShadowMapExtra1Center,_ShadowMapExtra1VP);
			extra1ShadowCoord.z = max(extra1ShadowCoord.z,0.0001f);
			FLOAT extraShadow1 = SampleShadowMap2(TEX2D_PARAM(_ShadowMapExtra1Tex),extra1ShadowCoord);
			extraShadow1 = (extra1ShadowCoord.x<0||extra1ShadowCoord.x>1||extra1ShadowCoord.y<0||extra1ShadowCoord.y>1)?1:extraShadow1;
			DEBUG_CUSTOMDATA_PARAM(ExtraShadow1, extraShadow1)	
			dynamicShadow *= extraShadow1;
			shadow *= extraShadow1;
		}
		#endif//_SIMPLE_EXTRA_SHADOW
	#endif//_EXTRA_SHADOW

	#ifdef _SELF_SHADOW
		#ifdef _DEPTH_SHADOW
			UNITY_BRANCH
			if (_DepthShadowEnable)
			{
				FLOAT2 scrPos = FragData.ClipScreenPosition.zw;
				FLOAT4 svPos = FragData.CLIPSVPOSTION;
				FLOAT depth = (svPos.z/svPos.w) * 0.5 + 0.5;
				FLOAT2 depthuv = scrPos + 5*( _DepthShadowLightDir+_ShadowDircontrol.xy) * float2(0.001953, 0.001953);
#ifndef ANDROID
				depthuv.y = 1 - depthuv.y;
#endif
				FLOAT4 hair = SAMPLE_TEX2D(_DepthShadowTex, float2(depthuv.x,depthuv.y));		
		        FLOAT hairDepth =hair.g;
				FLOAT depthCorrect = depth >= hairDepth+0.1+_ShadowDircontrol.z? 1 : 0;		
				FragData.RIMMASK=1-depthCorrect;
				DEBUG_CUSTOMDATA_PARAM(SelfShadow, depthCorrect)
			 	SetSelfShadow(ShadowData, depthCorrect);
			}
		#else
			UNITY_BRANCH
			if (_SelfShadowEnable)
			{
				FLOAT selfShadow= CalculateSelfShadow(worldPos.xyz);
				DEBUG_CUSTOMDATA_PARAM(SelfShadow, selfShadow)
				selfShadow *= mask;
				selfShadow += 0.45;
				selfShadow = lerp(1, selfShadow, mask);
				shadow *= selfShadow;
	...
	//#ifdef _SELF_SHADOW2
	//	UNITY_BRANCH
	//	if(_SelfShadow2Value)
	//	{	
	//		FLOAT selfShadow = CalculateSelfShadow(worldPos.xyz - _ObjCenter) * mask;
	//		DEBUG_CUSTOMDATA_PARAM(SelfShadow, selfShadow)
	//		shadow *= selfShadow;
	//	}
	//#endif// _SELF_SHADOW //

	ShadowData.Shadow = saturate(FLOAT4(_ShadowFade + shadow*(1-_ShadowFade),shadow,dynamicShadow,mask));

	...
	inout FLOAT3 DirectDiffuse,inout FLOAT3 DirectSpecular DEBUG_ARGS)
{
	...
	//uv mask
	#define _InvOcc _Param2.x
	#define darkColor _Color1
	FLOAT2 uv = GET_FRAG_UV;
	...
	#define _SpecularShininess _Param1.x
	#define _SpecularAmp _Param1.y
	#define _SpecularStart _Param1.z
	#define _SpecularRange _Param1.w
	#define _NormalBlend _Param2.y
	#define _SecondNormalUpright _Param2.z
	...
	FLOAT specularFade = smoothstep(_SpecularStart, 2, uv.y);
...
void SM_SS_WIND(FFragData FragData,FMaterialData MaterialData,
	in FLOAT3 LightDir,in FLOAT3 LightColor,in FShadowData ShadowData,
	inout FLOAT3 DirectDiffuse,inout FLOAT3 DirectSpecular DEBUG_ARGS)
{
	FLOAT3 ss = FAST_SS_Wind(FragData,LightDir,LightColor,diffuseParamWind);
	//uv mask
	// #define _InvOcc _Param2.x
	#define darkColor _Color1
	FLOAT2 uv = GET_FRAG_UV;
	FLOAT colorMask = saturate((uv.y+darkColor.a));
	colorMask = pow(colorMask,1.2); //_InvOcc=2
	FLOAT3 color = MaterialData.DiffuseColor;
	color = lerp(color*darkColor.xyz,MaterialData.DiffuseColor,colorMask)*ss;
	// DirectDiffuse = color*Shadow;
	DirectDiffuse = lerp(_ShadowColor.xyz*GetFadeShadow(ShadowData)*color,color,GetSrcShadow(ShadowData));
	DEBUG_CUSTOMDATA_PARAM(DirectDiffuse, DirectDiffuse)

	#define _SpecularShininessW _Param6.x
	#define _SpecularAmpW _Param6.y
	#define _SpecularStartW _Param6.z
	#define _SpecularRangeW _Param6.w
	// #define _NormalBlendW _Param7.y
	// #define _SecondNormalUprightW _Param7.z
	...
	FLOAT specularFade = smoothstep(_SpecularStartW, 1, uv.y);
	FLOAT3 H = normalize(FragData.CameraVector + LightDir);
	FLOAT3 secondNormal = normalize(FLOAT3(FragData.CameraVector.x, 1, FragData.CameraVector.z)); //_SecondNormalUprightW=1
	FLOAT3 specuarlNormal = normalize(lerp( secondNormal, worldNormal, 0));//_NormalBlendW=0
	FLOAT NdotH = max(0, dot(specuarlNormal, H));
	FLOAT shininess =  pow(NdotH, _SpecularShininessW) * saturate(1- length(FragData.WorldPosition_CamRelative)*_SpecularRangeW);
	FLOAT specular = (shininess * _SpecularAmpW * specularFade);	
	DirectSpecular = specular * LightColor.xyz * MaterialData.DiffuseColor*GetFadeShadow(ShadowData);	
	DEBUG_CUSTOMDATA_PARAM(DirectSpecular, DirectSpecular)
}
...
#endif
	NoL = saturate(dot(MaterialData.WorldNormal, SpecLightDir.xyz));

	FLOAT3 S1 = StrandSpecular2(T1, H,_PrimaryExponent) * _PrimarySpecualrColor.rgb * _SpecualrIntensity1;
	FLOAT3 S2 = StrandSpecular2(T2, H,_SecondExponent) * _SecondSpecualrColor.rgb * _SpecualrIntensity2;
	FLOAT3 S = S1 + S2;
	...
	DEBUG_ARGS)
{
	...
	FLOAT darkRim = saturate(smoothstep(darkRimMin, darkRimMax, darkRimDegree)) * _DarkRimLightColor.w;
...
{
	#ifdef _DEPTH_SHADOW
		_RimCutLocation = FragData.RIMMASK > 0 ? 1 : _RimCutLocation;
	#endif
	...
	FLOAT rim = saturate(smoothstep(rimMin, rimMax, rimDegree)) * mask;

	FLOAT3 rimColor = _RoleRimColor * rim * _MainLightColor.xyz * lerp(ShadowMask, 1, _RoleRimFade) * intensity;

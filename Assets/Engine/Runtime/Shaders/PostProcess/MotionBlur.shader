Shader "Hidden/PostProcessing/MotionBlur"
{
    HLSLINCLUDE
        ...
        //TEX2D_SAMPLER(_CameraMotionVectorsTexture);
        //float4 _CameraMotionVectorsTexture_TexelSize;

        //// Packed velocity texture (2/10/10/10)
        //TEX2D_SAMPLER(_VelocityTex);
        //float2 _VelocityTex_TexelSize;

        //// NeighborMax texture
        //TEX2D_SAMPLER(_NeighborMaxTex);
		...
		half2 _BlurParam;//x:blurDiverge,y:matrix delta
        ...
        //    float2 v = SAMPLE_TEX2D(_CameraMotionVectorsTexture, i.texcoord).rg;
        ...
        //    half d = Linear01Depth(SAMPLE_DEPTH(_CameraDepthTexture, i.texcoord));
        ...
        //    half2 v1 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.xy).rg;
        //    half2 v2 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.zy).rg;
        //    half2 v3 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.xw).rg;
        //    half2 v4 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.zw).rg;
        ...
        //            vo = MaxV(vo, SAMPLE_TEX2D(_MainTex, uv).rg);
        //        }
        //    }
        ...
        //    half2 v1 = SAMPLE_TEX2D(_MainTex, i.texcoord - d.xy).rg;
        //    half2 v2 = SAMPLE_TEX2D(_MainTex, i.texcoord - d.wy).rg;
        //    half2 v3 = SAMPLE_TEX2D(_MainTex, i.texcoord - d.zy).rg;

        //    half2 v4 = SAMPLE_TEX2D(_MainTex, i.texcoord - d.xw).rg;
        //    half2 v5 = SAMPLE_TEX2D(_MainTex, i.texcoord).rg * cw;
        //    half2 v6 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.xw).rg;

        //    half2 v7 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.zy).rg;
        //    half2 v8 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.wy).rg;
        //    half2 v9 = SAMPLE_TEX2D(_MainTex, i.texcoord + d.xy).rg;
        ...
        //{
        //    half3 v = SAMPLE_TEX2D_LOD(_VelocityTex, uv, 0.0).xyz;
        ...
        //    const half4 c_p = SAMPLE_TEX2D(_MainTex, i.texcoord);
        ...
        //    const half2 v_max = SAMPLE_TEX2D(_NeighborMaxTex, i.texcoord + JitterTile(i.texcoord)).xy;
        ...
        //        // Color sample
        //        const half3 c = SAMPLE_TEX2D_LOD(_MainTex, uv0, 0.0).rgb;
		...
		half4 FragGhostBlur(VaryingsDefault i) : SV_Target
		{
			half zOverW = Linear01Depth(SAMPLE_DEPTH(_CameraDepthTexture, i.texcoord));
			half4 H = half4(i.texcoord.xy * 2 - 1, zOverW, 1);
			half4 D = mul(_InvVPMatrix,H);
			half4 worldPos = D / D.w;
			half4 currentPos = H;
			half4 previousPos = mul(_PreVPMatrix,worldPos);
			previousPos /= previousPos.w;
			half2 velocityRaw = (currentPos - previousPos).xy;
			half2 velocity = velocityRaw *_BlurParam.x;
			//velocity.y += _BlurParam.x;

			// Get the initial color at this pixel.
			half4 color = SAMPLE_TEX2D(_MainTex, i.texcoord);
			half2 texCoord = i.texcoord + velocity;
			half4 currentColor = SAMPLE_TEX2D(_MainTex, texCoord);
			color += currentColor;
			texCoord += velocity;
			//texCoord -= velocity.x + _BlurParam.x;

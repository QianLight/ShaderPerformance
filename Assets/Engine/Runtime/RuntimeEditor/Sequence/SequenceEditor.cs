...
namespace CFEngine
{
    ...
    public class SequenceEditor : EditorWindow
    {
        ...
        [System.NonSerialized] private SequenceComponent pickedComp;
        ...
        public float length
        {
            get { return sequence.duration; }
        }

        //The min view time
        public float viewTimeMin
        {
            get { return sequence.ViewTimeMin; }
            set { sequence.ViewTimeMin = value; }
        }

        //The max view time
        public float viewTimeMax
        {
            get { return sequence.ViewTimeMax; }
            set { sequence.ViewTimeMax = value; }
        }
        ...
        void DrawGuideLine(float time, Color color)
        {
            if (time >= viewTimeMin && time <= viewTimeMax)
            {
                ...
                GUI.color = color;
                GUI.DrawTexture(guideRect, SequenceStyles.whiteTexture);
        ...
        //    result.Add(0);
        //    result.Add(length);
        //    result.Add(sequence.CurrentTime);
        //    if (sequence.directorGroup != null)
        //    {
        ...
        public static void ShowWindow(Sequence s)
        {
            var window = EditorWindow.GetWindow(typeof(SequenceEditor)) as SequenceEditor;
            window.InitializeAll(s);
            window.Show();
        }
...
#pragma warning restore
        ...
        void InitializeAll() { InitializeAll(sequence); }
        void InitializeAll(Sequence s)
        {
            ...
            titleContent = new GUIContent(string.Format("Sequence:{0}", sequence != null ? sequence.name : "empty"), SequenceStyles.cutsceneIconOpen);
            willRepaint = true;
            //OnEnable();
        }
        ...
        void OnGUI() {
            ...
            ////...
            //DoKeyboardShortcuts();
            ShowPlaybackControls(ref topLeftRect);
            ShowTimeInfo(ref topMiddleRect);
            //ShowToolbar();
            DoScrubControls();
            DoZoomAndPan();
            ...
            ShowGroupsAndTracksList(ref leftRect);
        ...
        void DrawGuides()
        {
            ...
            {
                DrawGuideLine(sequence.CurrentTime, scruberColor);
            }
        ...
        void DoScrubControls()
        {
            ...
            if (e.type == EventType.MouseDown && topMiddleRect.Contains(mousePosition))
            {
                ...
                //if (e.button == 1 && isEndCarret && sequence.directables != null)
                //{
                //    var menu = new GenericMenu();
                //    menu.AddItem(new GUIContent("Set To Last Clip Time"), false, () =>
                //        {
                //            var lastClip = sequence.directables.Where(d => d is ActionClip).OrderBy(d => d.endTime).LastOrDefault();
                //            if (lastClip != null)
                //            {
            ...
            {
                sequence.CurrentTime = SnapTime(pointerTime);
                sequence.CurrentTime = Mathf.Clamp(sequence.CurrentTime, Mathf.Max(viewTimeMin, 0) + float.Epsilon, viewTimeMax - float.Epsilon);
            }
        ...
        //top left controls
        void ShowPlaybackControls(ref Rect topLeftRect)
        {

            if (!isProSkin) { GUI.contentColor = Color.black.WithAlpha(0.7f); }
            GUI.color = Color.white;
            var stopRect = new Rect(topLeftRect.xMin + 10, topLeftRect.yMin + 4, 80, 21);
            if (GUI.Button(stopRect, "Stop"))
            {
            
            }

            var playRect = new Rect(stopRect.xMax + 10, topLeftRect.yMin + 4, 80, 21);
            if (GUI.Button(playRect, "Play"))
            {

            }
            var lockTimeRect = new Rect(playRect.xMax + 10, topLeftRect.yMin + 4, 80, 21);
            if (GUI.Button(lockTimeRect, "LockTime"))
            {
        ...
        void ShowTimeInfo(ref Rect topMiddleRect)
        {
            ...
            GUI.Box(topMiddleRect, string.Empty, SequenceStyles.timeBoxStyle);
            GUI.color = Color.white;
            ...
            GUI.BeginGroup(topMiddleRect);
            {
                ...
                GUI.DrawTexture(Rect.MinMaxRect(0, TOP_MARGIN - 1, topMiddleRect.xMax, TOP_MARGIN), SequenceStyles.whiteTexture);
                GUI.color = Color.white;
                ...
                if (centerRect.width / (viewTime / timeStep) > 6)
                {
                    for (var i = timeInfoStart; i <= timeInfoEnd; i += timeStep)
                    {
                        ...
                        GUI.DrawTexture(frameRect, SequenceStyles.whiteTexture);
                        GUI.color = Color.white;
                    }
                }
                ...
                for (var i = timeInfoStart; i <= timeInfoEnd; i += timeInfoInterval)
                {
                    ...
                    GUI.DrawTexture(markRect, SequenceStyles.whiteTexture);
                    GUI.color = Color.white;
                ...
                if (sequence.CurrentTime > 0)
                {
                    var label = doFrames ? (sequence.CurrentTime * Prefs.frameRate).ToString("0") : sequence.CurrentTime.ToString("0.00");
                    var text = "<b><size=17>" + label + "</size></b>";
                    var size = SequenceStyles.headerBoxStyle.CalcSize(new GUIContent(text));
                    var posX = TimeToPos(sequence.CurrentTime);
                    var stampRect = new Rect(0, 0, size.x, size.y);
                    ...
                    GUI.Box(stampRect, text, SequenceStyles.headerBoxStyle);
                }
                ...
                GUI.color = isProSkin ? Color.white : Color.black;
                GUI.DrawTexture(lengthRect, SequenceStyles.carretIcon);
                GUI.color = Color.white;
            }
            GUI.EndGroup();
        }
        ...
        void ShowGroupsAndTracksList(ref Rect leftRect)
        {
        ...
        void ShowListGroups(Event e, ref float nextYPos)
        {
            ...
            //GROUPS
            for (int i = 0; i < sequence.components.Count; i++)
            {
                var comp = sequence.components[i];

                var yPos = nextYPos;

                var trackRect = new Rect(10, yPos, leftRect.width - TRACK_RIGHT_MARGIN - 10, comp.defaultHeight);
                nextYPos += comp.defaultHeight + TRACK_MARGINS;

                //GRAPHICS
                GUI.color = ColorUtilities.Grey(isProSkin ? 0.25f : 0.9f);
                GUI.DrawTexture(trackRect, SequenceStyles.whiteTexture);
                GUI.color = Color.white.WithAlpha(0.25f);
                GUI.Box(trackRect, string.Empty);

                GUI.BeginGroup(trackRect);
                comp.OnTrackInfoGUI(ref trackRect);
                GUI.EndGroup();

                //if (ReferenceEquals(comp, CutsceneUtility.selectedObject) || track == pickedTrack)
                //{
                //    GUI.color = LIST_SELECTION_COLOR;
                //    GUI.DrawTexture(trackRect, whiteTexture);
                //}

                //var compRect = new Rect(4, nextYPos, leftRect.width - GROUP_RIGHT_MARGIN - 4, GROUP_HEIGHT - 3);
                //this.AddCursorRect(compRect, pickedComp == null ? MouseCursor.Link : MouseCursor.MoveArrow);
                //nextYPos += GROUP_HEIGHT;

                /////highligh?
                //var groupSelected = (ReferenceEquals(comp, comp == pickedComp));
                //GUI.color = groupSelected ? LIST_SELECTION_COLOR : GROUP_COLOR;
                //GUI.Box(compRect, string.Empty, SequenceStyles.headerBoxStyle);
                //GUI.color = Color.white;


                //var plusClicked = false;
                //GUI.color = isProSkin ? Color.white.WithAlpha(0.5f) : new Color(0.2f, 0.2f, 0.2f);
                //var plusRect = new Rect(compRect.xMax - 14, compRect.y + 5, 8, 8);
                //if (GUI.Button(plusRect, SequenceStyles.plusIcon, GUIStyle.none)) { plusClicked = true; }
                //if (!group.isActive)
                //{
                ...
                //var foldRect = new Rect(compRect.x + 2, compRect.y + 1, 20, compRect.height);
        ...
        [MenuItem("Window/Engine/Open Sequence Editor", false, 0)]
        public static void OpenScequenceWindow()
        {
            ShowWindow(null);
        }
    }

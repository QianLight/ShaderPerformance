...
/// Designed by 张欣颖 黑仔
/// </summary>
public class ReciprocalBasedCurve
{
    /// <summary>
    /// 曲线的曲折度。
    /// </summary>
    ...
    public static readonly ReciprocalBasedCurve Default = Create(MIN_CURVENESS, true, false);

    /// <summary>
    /// 禁用所有构造函数以方便报错返回控制。
    /// </summary>
    private ReciprocalBasedCurve() { }
    ...
    /// <returns></returns>
    public static ReciprocalBasedCurve Create(float curveness, bool flipVertical, bool flipHorizontal)
    {
        if (curveness == 0)
        {
            Debug.LogError("Setting BlackCurve error : Curveness could not be zero.");
        ...
        ReciprocalBasedCurve curve = new ReciprocalBasedCurve();

...
namespace CFEngine.Editor
{
    ...
    public sealed class TrackballUIDrawer
    {
        ...
        public void OnGUI(ref Vector4 value, ref bool overrideState, GUIContent title, Func<Vector4, Vector3> computeFunc, bool drawOverride = false)
        {
            ...
            using (new EditorGUILayout.VerticalScope())
            {
                using (new EditorGUI.DisabledScope(!overrideState))
                    DrawWheel(ref value, overrideState);

                DrawLabelAndOverride(title, ref overrideState, drawOverride);
            }
        ...
        void DrawLabelAndOverride(GUIContent title, ref bool overrideState,bool drawOverride)
        {
            // Title
            ...
            if(drawOverride)
            {
                ...
                if(smallTickbox == null)
                    smallTickbox = new GUIStyle("ShurikenToggle");
                overrideState = GUI.Toggle(overrideRect, overrideState, EditorGUIUtility.TrTextContent("", "Override this setting for this volume."), smallTickbox);
            }
        }

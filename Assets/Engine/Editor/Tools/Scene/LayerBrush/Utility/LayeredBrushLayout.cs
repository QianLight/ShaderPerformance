...
namespace CFEngine.Editor
{
	...
	/// </summary>
	internal static class LayeredBrushLayout
	{
		...
		{
			return ColorMaskField (string.IsNullOrEmpty (text) ? null : LayeredBrushGUI.TempContent (text), mask);
		}
		...
		{
			GUIContent gc = LayeredBrushGUI.TempContent ("", tooltip);
		...
		{
			return FreeSlider (LayeredBrushGUI.TempContent (content), value, min, max);
		}
		...
		internal static int ChannelField (int index, List<Texture2D> showtex, int length, Material mat, out Rect r)
		{
			r = GUILayoutUtility.GetLastRect ();
			int yPos = (int) Mathf.Ceil (r.y + r.height);

			index = ChannelField (index, showtex, 64, yPos, length, true, mat);

			return index;
		}
		internal static void ChannelField2 (int index, List<Texture2D> showtex, int length, Material mat, out Rect r)
		{
			r = GUILayoutUtility.GetLastRect ();
			int yPos = (int) Mathf.Ceil (r.y + r.height);
			ChannelField (index, showtex, 64, yPos, length, false, mat);
		}
		internal static int ChannelField (int index, List<Texture2D> showtex, int thumbSize, int yPos, int length, bool needSelect, Material mat = null)
		{
			int mIndex = index;
			int attribsLength = length;
			...
			// Draw background
			EditorGUI.DrawRect (border, EditorGUIUtility.isProSkin ? LayeredBrushGUI.k_BoxOutlineDark : LayeredBrushGUI.k_BoxOutlineLight);
			...
			for (int i = 0; i < attribsLength; i++)
			{
				...
				if (string.IsNullOrEmpty (summary))
					summary = "channel\n" + (i + 1);
				if (AttributeComponentButton (r, summary, showtex[i], i == mIndex, needSelect, mat))
				{
					mIndex = i;
					GUI.changed = true;
				}
		...
		internal static int FindTexIndex (Vector2 pos, Rect lastR, List<Texture2D> showtex, ref Rect selectRect)
		{
			int yPos = (int) Mathf.Ceil (lastR.y + lastR.height);

			int attribsLength = showtex.Count;

			const int margin = 4; // group pad
			const int pad = 2; // texture pad
			const int selected_rect_height = 10; // the little green bar and height padding

			int actual_width = 64 + pad;
			int container_width = (int) Mathf.Floor (EditorGUIUtility.currentViewWidth) - ((margin + pad) * 2);
			...
			if (border.Contains (pos))
			{
				...
				for (int i = 0; i < attribsLength; i++)
				{
					...
					Rect selectborder = new Rect (r.x + 2, r.y + 6, r.width - 4, r.height - 4);

					selectborder.x += 2;
					selectborder.y += 2;
					selectborder.width -= 4;
					selectborder.height -= 4;

					if (selectborder.Contains (pos))
					{
						selectRect = selectborder;
						return i;
					}
		...
		static bool AttributeComponentButton (Rect rect, string text, Texture2D img, bool selected, bool needSelect, Material mat = null)
		{
			bool clicked = false;
			...
			if (img != null)
			{
				EditorGUI.DrawPreviewTexture (border, img, mat, ScaleMode.ScaleToFit, 0f);
			}
			else
			{
				EditorGUI.DrawRect (border, texture_button_fill);
				GUI.Label (border, text, LayeredBrushGUI.CenteredStyle);
			}
		...
		internal static int IntFieldWithSlider (GUIContent gc, int value, int minValue, int maxValue)
		{
			using (new GUILayout.HorizontalScope ())
			{
				...
				EditorGUILayout.LabelField (gc, GUILayout.Width (100));
				returnValue = EditorGUILayout.IntSlider (returnValue, minValue, maxValue);
				returnValue = Mathf.Clamp (returnValue, minValue, maxValue);
		...
		internal static Color ColorField (GUIContent gc, Color color)
		{
			...
			var ret = EditorGUILayout.ColorField (LayeredBrushGUI.TempContent ("", gc.tooltip), color);
			return ret;
		}
		...
		internal static Gradient GradientField (GUIContent gc, Gradient value)
		{
			...
			object out_gradient = EditorGUILayout.GradientField (LayeredBrushGUI.TempContent ("", gc.tooltip), value, null);

			return (Gradient) out_gradient;
		}

		internal static void Header (GUIContent gc)
		{
			using (new GUILayout.HorizontalScope ())
			{

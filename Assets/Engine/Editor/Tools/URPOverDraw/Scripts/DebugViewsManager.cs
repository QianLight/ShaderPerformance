...
namespace URPDebugViews
{
    public class DebugViewsManager
    {
        ...
        public void GetShadowsDistances(out Vector4 shadowDistances)
        {
            ...
            switch (urpAsset.shadowCascadeOption)
            {
        ...
        public void EnableViewWithMaterialName()
        {
            if (!CheckForCorrectlyLoaded())
                return;

            string viewName = is_show ? "None" : "Overdraw";
            is_show = !is_show;

            //Debug.LogError(_debugViewDatas[0].MatTransparent.name);

            var selectedData = _debugViewDatas.FirstOrDefault(data => data.name == viewName);
            //Debug.LogError(selectedData.name);
            EnableView(selectedData);
        }
    }
}
...
namespace CFEngine.Editor
{
    ...
    public sealed class CurveEditor
    {
        ...
        public CurveEditor () : this (Settings.defaultSettings) { }
        ...
        public void Set (AnimationCurve curve, ref CurveState state)
        {
            this.curve = curve;
            this.state = state;
        }
        ...
        public void SetCurveState (bool edit)
        {
            state.editable = edit;
        }
        ...
        {
            var animCurve = curve;
            DoSetKeyframe (animCurve, keyframeIndex, keyframe);
        ...
        void OnGeneralUI ()
        {
            ...
            if (e.type == EventType.KeyDown && (e.keyCode == KeyCode.Delete || e.keyCode == KeyCode.Backspace))
            {
                ...
                {
                    var animCurve = m_SelectedCurve;
                    var length = animCurve.length;

                    if (state.minPointCount < length && length >= 0)
                    {
        ...
        void SaveCurve (AnimationCurve prop, AnimationCurve curve)
        {
            prop = curve;
        }
        ...
        void DoSetKeyframe (AnimationCurve curve, int keyframeIndex, Keyframe newValue)
        {
            var keys = curve.keys;
        ...
        void EditMoveKeyframe (AnimationCurve curve, Keyframe[] keys, int keyframeIndex)
        {
            ...
            DoSetKeyframe (curve, keyframeIndex, new Keyframe (key.x, key.y, inTgt, outTgt));
        }
        ...
        void EditMoveTangent (AnimationCurve curve, Keyframe[] keys, int keyframeIndex, Tangent targetTangent, bool linkTangents)
        {
            ...
            DoSetKeyframe (curve, keyframeIndex, new Keyframe (time, value, inTangent, outTangent));
        }

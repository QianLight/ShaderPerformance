...
namespace CFEngine.Editor
{
    ...
    class ElementResizer : Manipulator
    {
        ...
        void OnMouseMove(MouseMoveEvent e)
        {
            ...
            if (!m_DragStarted)
            {
                if (resizedTarget is IGraphResizable)
                {
                    (resizedTarget as IGraphResizable).OnStartResize();
                }
                m_DragStarted = true;
            }
        ...
        void OnMouseUp(MouseUpEvent e)
        {
            if (e.button == 0)
            {
                ...
                if (resizedTarget.style.width != m_StartSize.x || resizedTarget.style.height != m_StartSize.y)
                {
                    if (resizedTarget is IGraphResizable)
                    {
                        (resizedTarget as IGraphResizable).OnResized();
                    }
                }
    ...
    class ResizableElement : VisualElement
    {
        ...
        {
            var tpl = CanvasView.LoadUXML(uiFile);
            var sheet = CanvasView.LoadStyle("Resizable");
    ...
    class StickyNote : GraphElement, IGraphResizable
    {
        public enum Theme
        {
            Classic,
        ...
        {
            styleSheets.Add(CanvasView.LoadStyle("Selectable"));
            styleSheets.Add(CanvasView.LoadStyle("StickyNote"));
        ...
        public System.Action<StickyNodeChangeEvent.Change> OnChange = null;

...
namespace CFEngine.Editor
{
    ...
    public class NodeView : Node, ICanDirty
    {
        ...
        public Dictionary<string, VisualElement> edits = new Dictionary<string, VisualElement> ();
        ...
        protected void AddEditableField (EditableReflectionData editable)
        {
            ...
            if (edits.ContainsKey (editable.key))
            {
                DebugLog.AddEngineLog2 ("key duplicate:{0}", editable.key);
            }
            else
            {
                edits.Add (editable.key, field);
            }

...
namespace UnityEditor.Timeline
{
    static class SequencerContextMenu
    {
        ...
        public static void AddClipMenuCommands (List<MenuActionItem> menuItems, ICollection<TrackAsset> tracks, double candidateTime)
        {
            if (!tracks.Any ())
                return;
            ...
            foreach (var assetType in visibleAssetTypes)
            {
                ...
                if (Attribute.GetCustomAttribute (assetType, typeof (CSDiscriptor)) is CSDiscriptor attr)
                {
                    string pref = attr.classify;
                    if (!string.IsNullOrEmpty (pref))
                    {
                        pref += "/";
                    }

                    commandName = pref + commandName + " [" + attr.discriptor + "]";
                }
                GenericMenu.MenuFunction command = () =>
                {
                    foreach (var t in tracks)
        ...
        public static void AddMarkerMenuCommands (List<MenuActionItem> menu, IEnumerable<Type> markerTypes, Action<Type, Object> addMarkerCommand, bool enabled)
        {
            ...
            foreach (var markerType in markerTypes)
            {
                ...
                menu.Add (
                    ...
                    {
                        category = category,
                            entryName = string.Format (Styles.addItemTemplate, TypeUtility.GetDisplayName (markerType)) + TimelineHelpers.GetTrackDecriptor(markerType),
                            shortCut = string.Empty,
                            isActiveInMode = true,
                            isChecked = false,

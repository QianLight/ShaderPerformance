...
namespace UnityEditor.Timeline
{
    ...
    [MenuEntry("SelectAll", MenuOrder.TimelineAction.MatchContent)]
    //[Shortcut(Shortcuts.Timeline.matchContent)]
    class SelectAll : TimelineAction
    {
        public override MenuActionDisplayState GetDisplayState(WindowState state)
        {
            return MenuActionDisplayState.Visible;
        ...
        public override bool Execute(WindowState state)
        {
            //UnityEngine.Playables.PlayableDirector director = GameObject.Find("timeline").GetComponent<UnityEngine.Playables.PlayableDirector>();
            UnityEngine.Playables.PlayableAsset asset = state.editSequence.asset; //director.playableAsset;
            foreach (UnityEngine.Playables.PlayableBinding pb in asset.outputs)
            {
                if (pb.sourceObject is TrackAsset)
                {
                    TrackAsset ta = pb.sourceObject as TrackAsset;
                    TimelineClip[] clips = ta.clips;
                    for (int i = 0; i < clips.Length; ++i)
                    {
                        //if (clips[i].start <= 0) continue;
                        SelectionManager.AddObject(EditorClipFactory.GetEditorClip(clips[i]));
                    }
                    if(ta is AnimationTrack)
                    {
                        AnimationTrack animationTrack = ta as AnimationTrack;
                        if(animationTrack.infiniteClip != null)
                        {
                            SelectionManager.AddObject(ta);
                        }
                    }
                }
            }
    ...
    [MenuEntry("SelectCurrentTrack", MenuOrder.TimelineAction.MatchContent)]
    //[Shortcut(Shortcuts.Timeline.matchContent)]
    class SelectCurrentTrack : TimelineAction
    {
        public override MenuActionDisplayState GetDisplayState(WindowState state)
        {
            return MenuActionDisplayState.Visible;
        ...
        public override bool Execute(WindowState state)
        {
            ...
            EditorClip editorClip = obj as EditorClip;
            if(editorClip != null)
            {
                TrackAsset ta = editorClip.clip.parentTrack as TrackAsset;
                TimelineClip[] clips = ta.clips;
                for (int i = 0; i < clips.Length; ++i)
                {
                    SelectionManager.AddObject(EditorClipFactory.GetEditorClip(clips[i]));

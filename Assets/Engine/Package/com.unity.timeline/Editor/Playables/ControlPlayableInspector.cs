...
namespace UnityEditor.Timeline
{
    ...
    class ControlPlayableInspector : Editor
    {
        ...
        SerializedProperty m_roleIndex;
        ...
        public void OnEnable()
        {
            ...
            m_roleIndex = serializedObject.FindProperty("m_roleIndex");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(m_roleIndex, new GUIContent("RoleIndex"));

            var asset = target as ControlPlayableAsset;
            var dat = asset.fxData;
            if (dat != null && !string.IsNullOrEmpty(dat.path))
            {
                GUILayout.Space(8);
                EditorGUILayout.LabelField("Fx Info: ");
                EditorGUILayout.LabelField(dat.path);
                EditorGUILayout.LabelField("pos: \t" + dat.pos.ToString("f1"));
                EditorGUILayout.LabelField("rot: \t" + dat.rot.ToString("f1"));
                EditorGUILayout.LabelField("scale: \t" + dat.scale.ToString("f1"));
                EditorGUILayout.LabelField("load time: \t" + dat.loadtime);
                if (!string.IsNullOrEmpty(dat.avatar))
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.LabelField("bind: \t" + dat.avatar);
                }
            }

...
namespace UnityEditor.Timeline
{
    ...
    class AnimationPlayableAssetInspector : Editor
    {
        ...
        public override void OnInspectorGUI ()
        {
            ...
            uint value = (uint) flag.intValue;
            EditorGUI.BeginChangeCheck ();
            bool overrideAnim = EditorGUILayout.Toggle ("OverrideAnim", (value & DirectorAnimationPlayableAsset.Flag_Override) != 0);
            if (EditorGUI.EndChangeCheck ())
            {
                if (overrideAnim)
                {
                    value |= DirectorAnimationPlayableAsset.Flag_Override;
                }
                else
                {
                    value &= ~DirectorAnimationPlayableAsset.Flag_Override;
                }
                flag.intValue = (int) value;
            }
            serializedObject.ApplyModifiedProperties ();
        }

...
namespace UnityEditor.Timeline
{
    partial class TimelineWindow
    {
        ...
        private string m_moveStartStr = string.Empty;
        private string m_moveEndStr = string.Empty;
        private string m_moveTimeStr = string.Empty;
        void MoveAreaGUI()
        {
            GUILayout.Space(10);
            GUILayout.Label("s");
            m_moveStartStr = GUILayout.TextField(m_moveStartStr, GUILayout.Width(50));
            GUILayout.Label("e");
            m_moveEndStr = GUILayout.TextField(m_moveEndStr, GUILayout.Width(50));
            GUILayout.Label("t");
            m_moveTimeStr = GUILayout.TextField(m_moveTimeStr, GUILayout.Width(50));
            float start = (float)TimeReferenceUtility.FromTimeString2(m_moveStartStr);
            float end = (float)TimeReferenceUtility.FromTimeString2(m_moveEndStr);
            float deltaTime = (float)TimeReferenceUtility.FromTimeString2(m_moveTimeStr);

            if (GUILayout.Button("movearea"))
            {
                DoMoveArea(start, end, deltaTime);
                //m_moveStartStr = string.Empty;
                //m_moveEndStr = string.Empty;
                //m_moveTimeStr = string.Empty;
            }
        }

        private void DoMoveArea(float start, float end, float deltaTime)
        {
            UnityEngine.Playables.PlayableDirector director = GameObject.FindObjectOfType<UnityEngine.Playables.PlayableDirector>();
            var tracks = (director.playableAsset as TimelineAsset).GetRootTracks();
            foreach (var track in tracks)
            {
                DoMoveTrackAsset(track, start, end, deltaTime);
            }
        }

        /// <summary>
        /// 可能会有多层的group，所以使用递归的方法进行移动
        /// </summary>
        /// <param name="trackAsset"></param>
        /// <param name="start"></param>
        /// <param name="end"></param>
        /// <param name="deltaTime"></param>
        private void DoMoveTrackAsset(TrackAsset trackAsset, float start, float end, float deltaTime)
        {
            var childTracks = trackAsset.GetChildTracks();
            foreach (var item in childTracks)
            {
                DoMoveTrackAsset(item, start, end, deltaTime);
            }
            ...
            foreach (var item in clips)
            {
                ...
                {
                    TimelineUndo.PushUndo(trackAsset, "DoMoveClip");
            ...
            {
                DoMoveAreaCurve(start, end, deltaTime, animationTrack);
            }
        }

        private void DoMoveAreaCurve(float start, float end, float deltaTime, AnimationTrack track)
        {
            if (track == null || track.infiniteClip == null) return;
            AnimationClip animClip = track.infiniteClip;
            ...
            for (int i = 0; i < curveBinding.Length; ++i)
            {
                ...
                List<Keyframe> allFrames = new List<Keyframe>();
                for (int j = 0; j < keyframes.Length; ++j)
                {
                    float time = keyframes[j].time;
                    if (time >= start && time <= end)
                    {
                        keyframes[j].time += deltaTime;
                    }
                    bool exist = false;
                    for (int k = 0; k < allFrames.Count; ++k)
                    {
                        if (Mathf.Abs(allFrames[k].time - keyframes[j].time) < 0.01f)
                        {
                            exist = true;
                        }
                    }
                    if (!exist) allFrames.Add(keyframes[j]);
                }

                curve.keys = allFrames.ToArray();
                Undo.RegisterCompleteObjectUndo(animClip, "domovecurve");
                AnimationUtility.SetEditorCurve(animClip, curveBinding[i], curve);
            }
        }


        private string m_moveTime = string.Empty;
        private float m_deltaTime = 0;
        void MoveGUI()
        {
            m_moveTime = GUILayout.TextField(m_moveTime, GUILayout.Width(50));
            m_deltaTime = (float)TimeReferenceUtility.FromTimeString2(m_moveTime);
            if (GUILayout.Button("move"))
            {
                DoMoveClip();
                DoMoveCurve();
                m_moveTime = string.Empty;
                m_deltaTime = 0;
            }
        }

        private void DoMoveClip()
        {
            var clips = SelectionManager.SelectedClips();
            foreach (var clip in clips)
            {
                TimelineUndo.PushUndo(clip.parentTrack, "DoMoveClip");
                clip.start += m_deltaTime;
            }
        }

        private void DoMoveCurve()
        {
            var tracks = SelectionManager.SelectedTracks();
            foreach (var track in tracks)
            {
                DoMoveCurve(track as AnimationTrack);
            }
        }

        private void DoMoveCurve(AnimationTrack track)
        {
            if (track == null || track.infiniteClip == null) return;
            AnimationClip animClip = track.infiniteClip;
            EditorCurveBinding[] curveBinding = AnimationUtility.GetCurveBindings(animClip);

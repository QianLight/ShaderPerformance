...
namespace UnityEditor.Timeline
{
    class TimelineDataSource : TreeViewDataSource
    {
        ...
        public sealed override void FetchData()
        {
            ...
            if (TimelineExternGUI.showEnv)
            {
                var engineTrack = m_TimelineWindow.state.editSequence.asset.engineTrack;
                if (engineTrack != null)
                {
                    CreateItem(engineTrack, ref tree, filteredView.OfType<TrackAsset>(), m_RootItem);
                }
            }

...
{
    public delegate IAnimEnv GetAnimEnv(GameObject go);
    /// <summary>
    ...
    public partial class TimelineAsset : PlayableAsset, ISerializationCallbackReceiver, ITimelineClipAsset, IPropertyPreview
    {
        ...
        public static GetAnimEnv getAnimEnv = null;
        internal IAnimEnv animEnv = null;
#if UNITY_EDITOR
        [HideInInspector, NonSerialized] public EngineTrack m_EngineTrack;
#endif
        /// <summary>
        ...
        internal ScriptableObject engineTrack
        {
            get
            {
                if (m_EngineTrack == null || rebuild)
                {
                    m_EngineTrack = EngineTrack.CreateInstance<EngineTrack>();
                    m_EngineTrack.parent = this;
                    m_EngineTrack.name = "Engine";
                    rebuild = false;
                }
        ...
        public ScriptableObject GetTrackByIndex(int i)
        {
            if (i >= 0 && i < m_Tracks.Count) return m_Tracks[i];
            return null;
        }
#endif

        public IAnimEnv BindEngineObject()
        {
            if (getAnimEnv != null && timelineGo != null)
            {
                animEnv = getAnimEnv(timelineGo);
            }
            return animEnv;
        }
        ...
        public override Playable CreatePlayable(PlayableGraph graph, GameObject go)
        {
            ...
            var timeline = TimelinePlayable.Create(graph, this, GetOutputTracks(), go, autoRebalanceTree, createOutputs);

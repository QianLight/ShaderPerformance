...
namespace UnityEngine.Timeline
{
    ...
    public partial class ControlPlayableAsset : PlayableAsset, IPropertyPreview, ITimelineClipAsset
    {
        ...
        [SerializeField] public ControlFxData fxData;

        [SerializeField] public int m_roleIndex = -1;
        ...
        /// </summary>
        [SerializeField] public bool updateDirector = false;

        /// <summary>
        /// Indicates whether Monobehaviours implementing ITimeControl will be controlled.
        /// </summary>
        [SerializeField] public bool updateITimeControl = false;
        ...
        {
#if UNITY_EDITOR
            int cnt = mixer.GetInputCount();
            for (int i = 1; i < cnt; i++)
            {
                var control = mixer.GetInput(i);
                Debug.Log(i + " particle: " + " " + control.GetPlayableType());
                mixer.DisconnectInput(i);
            }
#endif
        }

        private void DisconnectParticles()
        {
            if (mixer.IsValid())
            {
                int cnt = mixer.GetInputCount();
        ...
        public void Rebuild(GameObject sourceObject, PlayableDirector director)
        {
            var particleSystems = GetParticleSystemRoots(sourceObject);
            ...
            if (mixer.IsValid())
            {
                ...
                var activation = ActivationControlPlayable.Create(graph, sourceObject, postPlayback);
                AddOrUpdateInput(0, cnt, activation);
                for (int i = 0; i < particleSystems.Count; i++)
                {
                    var particleSystem = particleSystems[i];
                    var playable = ParticleControlPlayable.Create(graph, particleSystem, particleRandomSeed);
                    AddOrUpdateInput(i + 1, cnt, playable);
                }
                mixer.SetPropagateSetTime(true);
            }
        }

        private void AddOrUpdateInput(int i, int cnt, Playable playable)
        {
            if (i < cnt)
            {

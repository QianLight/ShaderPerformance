...
namespace Cinemachine
{
    ...
    public struct AxisState
    {
        ...
        /// <summary>
        /// ��ǰ�������Ƿ�����Ч�ģ�������Ϊm_InputAxisValueΪ0��һ������Ч����������ͣ
        /// </summary>
        public bool m_InputAxisValueValid;
            ...
            string name, bool invert)
        {
            ...
            m_InputAxisValueValid = false;
        }
        public void InterruptAxisRecenter()
        {
            m_Recentering.CalcalateWaitTime();
        }
        ...
        public bool Update(float deltaTime)
        {
            ...
            else if (!string.IsNullOrEmpty(m_InputAxisName))
            {
                ...
                catch (ArgumentException e) { CFUtilPoolLib.XDebug.singleton.AddErrorLog(e.ToString()); }
            }
            ...
            return Mathf.Abs(input) > Epsilon || m_InputAxisValueValid;
        }
        ...
        public struct Recentering
        {
            ...
            [Tooltip("如果被其他操作打断，将用这个时间值重置等待时间")]
            public float m_interruptResetTime;
            ...
            public void CalcalateInputWaitTime()
            {
                //CinemachineCore.CurrentTime < (mLastAxisInputTime + realWaitTime)
                float remainTime = mLastAxisInputTime + realWaitTime - CinemachineCore.CurrentTime;
                if (remainTime < m_WaitTime) realWaitTime += (m_WaitTime - remainTime);//ʣ���ʱ�����ٱ�m_interruptResetTimeʱ�䳤
            }
            public void CalcalateWaitTime()
            {
                if (!IsBeforeRecentering) return;
                //CinemachineCore.CurrentTime < (mLastAxisInputTime + realWaitTime)
                float remainTime = mLastAxisInputTime + realWaitTime - CinemachineCore.CurrentTime;
                if (remainTime < m_interruptResetTime) realWaitTime += (m_interruptResetTime - remainTime);//ʣ���ʱ�����ٱ�m_interruptResetTimeʱ�䳤
            }
            private void ResetWaitTime()
            {
                realWaitTime = m_WaitTime;
            }
            private bool IsBeforeRecentering { get; set; }
            ...
            public void DoRecentering(ref AxisState axis, float deltaTime, float recenterTarget)
            {
                ...
                if (delta == 0)
                    return;
                IsBeforeRecentering = true;
                if (CinemachineCore.CurrentTime < (mLastAxisInputTime + realWaitTime))
                    return;
                IsBeforeRecentering = false;
                ResetWaitTime();

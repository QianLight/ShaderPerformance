...
namespace Cinemachine
{
    ...
    public sealed class CinemachineCore
    {
        ...
        private Dictionary<ICinemachineCamera, CameraLiveState> cameraState;
        private Dictionary<ICinemachineCamera, CameraLiveState> CameraState
        {
            get
            {
                if (cameraState == null) { cameraState = new Dictionary<ICinemachineCamera, CameraLiveState>();}
                if (Time.frameCount - lastClearFrame > 10000 || cameraState.Count > 50) { cameraState.Clear(); lastClearFrame = Time.frameCount; }
                return cameraState;
            }
        }
        ...
        public bool IsLive(ICinemachineCamera vcam)
        {
            if (vcam != null)
            {
                ...
                bool cacheContainsVcam = CameraState.TryGetValue(vcam, out CameraLiveState state);
                ...
                else
                {
                    ...
                    if (cacheContainsVcam)
                    {
                        ...
                        CameraState.Add(vcam, new CameraLiveState(frameCount, isLive));
                    return isLive;
                }
            }
            return false;
            ...
            if (vcam != null)
            {
                ...
                for (int i = 0; i < numBrains; ++i)
                {
                    ...
                    if (b != null && b.OutputCamera != null)
                    {
                        if (b.IsLive(vcam))
                            return b;
                        if ((b.OutputCamera.cullingMask & layer) != 0)
                            return b;
                    }
                }
            }

...
namespace Cinemachine
{
    ...
    public class CinemachineBlend
    {
        ...
        private CameraState LerpLastCamAState { get; set; }

        /// <summary>Second camera in the blend</summary>
        public ICinemachineCamera CamB { get; set; }
        private CameraState LerpLastCamBState { get; set; }
        ...
        private float LastLerpBlendWeight = -1;
        ...
        {
            LerpLastCamAState = CameraState.Default; CamA = a;
            LerpLastCamBState = CameraState.Default; CamB = b;
            BlendCurve = curve;
            TimeInBlend = t;
            Duration = duration;
        }
        ...
        public CameraState CacheLerpState = CameraState.Default;
        ...
        public CameraState State
        {
            get
            {
                ...
                if (CameraState.LerpEqual(CamA.State, LerpLastCamAState) && CameraState.LerpEqual(CamB.State, LerpLastCamBState) && Mathf.Abs(weight - LastLerpBlendWeight) < float.Epsilon)
                {
                    return CacheLerpState;
                }
                LerpLastCamAState = CamA.State; LerpLastCamBState = CamB.State; LastLerpBlendWeight = weight;
                CacheLerpState = CameraState.Lerp(CamA.State, CamB.State, weight);
                return CacheLerpState;
            }
        }
    }

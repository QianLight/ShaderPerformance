...
public sealed class CinemachineMixer : PlayableBehaviour
{
    ...
    public delegate bool GetEditorVCInfoDelegate(out CinemachineBlendDefinition.Style style1, out float t1, 
        out CinemachineVirtualCameraBase v1,
        out CinemachineBlendDefinition.Style style2, out float t2,
        out CinemachineVirtualCameraBase v2);
    static public MasterDirectorDelegate GetMasterPlayableDirector;
    public static GetEditorVCInfoDelegate GetEditorVCInfo;
    ...
    public override void ProcessFrame(Playable playable, FrameData info, object playerData)
    {
        ...
        CinemachineShotPlayable shot_p = null;
        for (int i = 0; i < inputCount; ++i)
        {
            float weight = playable.GetInputWeight(i);
            var clip = (ScriptPlayable<CinemachineShotPlayable>)playable.GetInput(i);
            shot_p = clip.GetBehaviour();
            if (shot_p != null && shot_p.IsValid
        ...
        // for editor preview
        if (GetMasterPlayableDirector != null && GetEditorVCInfo != null)
        {
            var d = GetMasterPlayableDirector();
            if (d != null)
            {
                if (GetEditorVCInfo(out var style1, out var d1, out var v1, out var style2, out var d2, out var v2))
                {
                    if (d.time < d1 && v1 != null && activeInputs == 1)
                    {
                        camA = v1;
                        weightB = (float)d.time / d1;
                        mBrain.m_DefaultBlend.m_Style = style1;
                        mBrain.m_DefaultBlend.m_Time = d1;
                    }
                }

                if (!Application.isPlaying)
                {
                    if (d.time > d.duration && v2 != null && activeInputs == 0)
                    {
                        if (d.time - d.duration < d2)
                        {
                            camA = shot_p.VirtualCamera;
                            camB = v2;
                            weightB = (float)(d.time - d.duration) / d2;
                            mBrain.m_DefaultBlend.m_Style = style2;
                            mBrain.m_DefaultBlend.m_Time = d2;
                        }
                        else if (d.time - d.duration > d2)
                        {
                            camA = null;
                            camB = v2;
                            weightB = 1;

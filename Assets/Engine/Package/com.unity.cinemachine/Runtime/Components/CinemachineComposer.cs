...
namespace Cinemachine
{
    ...
    public class CinemachineComposer : CinemachineComponentBase
    {
        ...
        public Vector3 m_TrackedObjectOffsetMin = Vector3.zero;

        public Vector3 m_TrackedObjectOffsetReadOnly = Vector3.zero;
        ...
        {
            get { return Mathf.Min(1, m_ScreenY * _screen_y_scale); }
            set { m_ScreenY = value; }
        }

        private float _screen_y_scale = 1f;
        public void SetCustomScale(float screenY)
        {
            _screen_y_scale = screenY;
        }
        #endregion
        ...
        protected Vector3 GetLookAtPointAndSetTrackedPoint(
            Vector3 finalPos, Vector3 rawPos,
            Vector3 lookAt, Vector3 up, float deltaTime)
        {
            Vector3 pos = lookAt;
            if (LookAtTarget != null)
                pos += LookAtTargetRotation * m_TrackedObjectOffset;
            {
                float actualDis = (lookAt - finalPos).magnitude;
                float totalDis = (lookAt - rawPos).magnitude;
                if (totalDis == 0.0f)
                    totalDis = 1.0f;
                m_TrackedObjectOffsetReadOnly = m_TrackedObjectOffsetMin + (m_TrackedObjectOffset - m_TrackedObjectOffsetMin) * (actualDis / totalDis);
                pos += LookAtTargetRotation * m_TrackedObjectOffsetReadOnly;
            }
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            curState.ReferenceLookAt = GetLookAtPointAndSetTrackedPoint(curState.CorrectedPosition, curState.RawPosition,
                curState.ReferenceLookAt, curState.ReferenceUp, deltaTime);
        ...
        internal Rect SoftGuideRect
        {
            ...
            {
                return new Rect(
                    m_ScreenX - m_DeadZoneWidth / 2, ScreenY - m_DeadZoneHeight / 2,
            ...
            set
            {
                ...
                ScreenY = Mathf.Clamp(value.y + m_DeadZoneHeight / 2, -0.5f,  1.5f);
        ...
        internal Rect HardGuideRect
        {
            ...
            {
                Rect r = new Rect(
                        m_ScreenX - m_SoftZoneWidth / 2, ScreenY - m_SoftZoneHeight / 2,
            ...
            set
            {
                ...
                Vector2 bias = center - new Vector2(m_ScreenX, ScreenY);

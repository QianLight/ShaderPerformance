    ...
    public class CinemachineImpulseManager
    {
        ...
        public class ImpulseEvent
        {
            ...
            public bool GetDecayedSignal(
                Vector3 listenerPosition, Quaternion listenerRotation, bool use2D, out Vector3 pos, out Quaternion rot)
            {
                if (m_SignalSource != null)
                {
                    float distance = Mathf.Max(0, (use2D ? Vector2.Distance(listenerPosition, m_Position)
                        : Vector3.Distance(listenerPosition, m_Position)) - m_Radius);
        ...
        public bool GetImpulseAt(
            Vector3 listenerLocation,Quaternion listenerRotation, bool distance2D, int channelMask,
            ...
            if (m_ActiveEvents != null)
            {
                for (int i = m_ActiveEvents.Count - 1; i >= 0; --i)
                {
                    ...
                    else if ((e.m_Channel & channelMask) != 0)
                    {
                        ...
                        if (e.GetDecayedSignal(listenerLocation, listenerRotation, distance2D, out pos0, out rot0))
                        {

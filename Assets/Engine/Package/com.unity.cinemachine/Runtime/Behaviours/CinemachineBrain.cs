...
//    [RequireComponent(typeof(Camera))] // strange but true: we can live without it
    ...
    public class CinemachineBrain : MonoBehaviour
    {
        ...
        public List<BrainFrame> mFrameStack = new List<BrainFrame>();
        ...
        public CinemachineBlend mCurrentLiveCameras = new CinemachineBlend(null, null, null, 0, 0);
            ...
            float weightB, float deltaTime)
        {
            ...
            frame.blend.BlendCurve = m_DefaultBlend.BlendCurve;
        ...
        public bool forceSetCam = false;
        public CinemachineVirtualCameraBase activeCam;
        ...
        private void UpdateFrame0(float deltaTime)
        {
            ...
            bool forceSetCamA = forceSetCam && activeCam != null && activeCam.transform.parent == null && activeCam.transform.position.y != -10000;
            if (!forceSetCamA)
            {
                forceSetCam = false;
                activeCam = null;
            }
            var activeCamera = forceSetCamA ? activeCam : TopCameraFromPriorityQueue();
        ...
        #region  Live 方法优化
        public int lastClearFrame = 0;
        private class CameraLiveState
        {
            public int frameCount;
            public bool live;
            public CameraLiveState(int frameCount, bool live)
            {
                this.frameCount = frameCount;
                this.live = live;
            }
        }
        private Dictionary<ICinemachineCamera, CameraLiveState> cinemachineLiveState;
        private Dictionary<ICinemachineCamera, CameraLiveState> CinemachineLiveState
        {
            get
            {
                if (cinemachineLiveState == null) { cinemachineLiveState = new Dictionary<ICinemachineCamera, CameraLiveState>(); }
                if (Time.frameCount - lastClearFrame > 10000 || cinemachineLiveState.Count > 50) { cinemachineLiveState.Clear(); lastClearFrame = Time.frameCount; }
                return cinemachineLiveState;
            }
        }
        ...
        public bool IsLive(ICinemachineCamera vcam, bool dominantChildOnly = false)
        {
            ...
            bool cacheContainsVcam = CinemachineLiveState.TryGetValue(vcam, out CameraLiveState state);
            if (cacheContainsVcam && state.frameCount == frameCount)
            {
                return state.live;
            }
            else
            {
                bool isLive = false;
                ICinemachineCamera key = vcam;
                if (SoloCamera == vcam)
                    isLive = true;
                else if (mCurrentLiveCameras.Uses(vcam))
                    isLive = true;
                else
                {
                ...
                if (cacheContainsVcam)
                {
                    state.live = isLive;
                    state.frameCount = frameCount;
                }
                else
                    CinemachineLiveState.Add(key, new CameraLiveState(frameCount, isLive));
                return isLive; 
            }
        }
        ...
        /// </summary>
        public ICinemachineCamera TopCameraFromPriorityQueue()
        {
        ...
        private void PushStateToUnityCamera(in CameraState state)
        {
            ...
            if ((state.BlendHint & CameraState.BlendHintValue.NoLens) == 0)
            {
                ...
                if (cam != null)
                {
                   ...
                   // cam.farClipPlane = state.Lens.FarClipPlane;//这行导致了相机farClip值被锁，所以注释掉了

...
namespace Cinemachine
{
    ...
    public class CinemachineVirtualCamera : CinemachineVirtualCameraBase
    {
        ...
        public float DetectDistane = 5;
        public bool LockPosY = false;
        public float minLockPosY = -1;
        public float maxLockPosY = -999;
        public float PosY = 0;
        public float RotX = 0;
        ...
        {
            CinemachineComponentBase component = GetCinemachineComponent(CinemachineCore.Stage.Aim);
            if (component is CinemachinePOV)
            {
                (component as CinemachinePOV).m_HorizontalRecentering.CalcalateWaitTime();
                (component as CinemachinePOV).m_VerticalRecentering.CalcalateWaitTime();
            }
        }
        ...
        override public void InternalUpdateCameraState(Vector3 worldUp, float deltaTime)
        {
            ...
            if (component is CinemachinePOV)
            {
                (component as CinemachinePOV).m_HorizontalAxis.m_InputAxisValue = m_InputX;
                (component as CinemachinePOV).m_HorizontalAxis.m_InputAxisValueValid = m_InputValid;
                (component as CinemachinePOV).m_VerticalAxis.m_InputAxisValue = m_InputY;
                (component as CinemachinePOV).m_VerticalAxis.m_InputAxisValueValid = m_InputValid;
                (component as CinemachinePOV).m_InputValid = m_InputValid;
            }
            ...
            if (LockPosY)
            {
                if(m_Follow != null)
                {
                    m_State.RawPosition = new Vector3(m_State.RawPosition.x, m_Follow.position.y + PosY, m_State.RawPosition.z);
                    if (LockRotY)
                        m_State.RawOrientation = Quaternion.Euler(RotX, RotY, m_State.RawOrientation.eulerAngles.z);
                    else 
                        m_State.RawOrientation = Quaternion.Euler(RotX, m_State.RawOrientation.eulerAngles.y, m_State.RawOrientation.eulerAngles.z);
                }
            }
            ...
            if (m_Follow != null)
            {
                ...
                {
                    float minY = m_Follow.position.y + MinYBasedOnFollow;
                ...
                {
                    float maxY = m_Follow.position.y + MaxYBasedOnFollow;
                    float nowY = m_State.CorrectedPosition.y;
                    if (nowY > maxY)
        ...
        void Reset()
        {
            ...
            {
                CFUtilPoolLib.XDebug.singleton.AddLog("You cannot reset a prefab instance.  "

...
namespace Cinemachine
{
    ...
    public class CinemachineFreeLook : CinemachineVirtualCameraBase
    {
        ...
        [HideInInspector]
        public float m_InputX = 0;
        [HideInInspector]
        public float m_InputY = 0;
        [HideInInspector]
        public bool m_ShouldResetSpeed = false;
        [HideInInspector]
        public bool m_InputScriptControl = false;
        [HideInInspector]
        public bool m_InputValid = false;
        ...
        [Range(0f, 1f)]
        public float m_FollowParam = 0f;
        [HideInInspector]
        public bool m_FollowFlag = true;

        public bool m_useDistance = false;

        [HideInInspector]
        public float _lookatPercent=1;
        public float MinYBasedOnFollow = 0.0f;
        public float MaxYBasedOnFollow = 0.0f;
        ...
        public struct Orbit
        {
            ...
            public float Height
            {
                get { return m_Height * m_Scale; }
                set { m_Height = value; }
            }
            /// <summary>Radius of orbit</summary>
            public float Radius
            {
                get { return m_Radius * m_Scale; }
                set { m_Radius = value; }
            }
            public float Scale
            {
            ...
            public Orbit(float h, float r) { m_Height = h; m_Radius = r; m_Scale = 1; }
        }
        ...
        CFUtilPoolLib.IXScene m_IXScene;
        void Awake()
        {
            m_IXScene = CFUtilPoolLib.XInterfaceMgr.singleton.GetInterface<CFUtilPoolLib.IXScene>(CFUtilPoolLib.XInterfaceMgr.XSceneID);
        }
        ...
        void Reset()
        {
            ...
            {
                XDebug.singleton.AddLog("You cannot reset a prefab instance.  "
        ...
        {
            if (m_InputScriptControl && enabled)
            {
                m_XAxis.m_InputAxisValue = m_InputX;
                m_YAxis.m_InputAxisValue = m_InputY;
                m_XAxis.m_InputAxisValueValid = m_InputValid;
                m_YAxis.m_InputAxisValueValid = m_InputValid;
                if (m_ShouldResetSpeed) { m_XAxis.Reset(); m_YAxis.Reset(); }
            }
            else
            {
                m_XAxis.Reset(); m_YAxis.Reset();
            }
            //更新3轨信息
            UpdateRigCache();
            ...
            float dy = m_State.ReferenceLookAt.y - m_State.RawPosition.y;
            float actualDis = (m_State.ReferenceLookAt - m_State.CorrectedPosition).magnitude;
            float totalDis = (m_State.ReferenceLookAt - m_State.RawPosition).magnitude;
            if (totalDis == 0.0f)
                totalDis = 1.0f;
            float ratio = actualDis / totalDis;
            //修正的是什么？
            m_State.PositionCorrection = new Vector3(m_State.PositionCorrection.x, dy * (1.0f - ratio), m_State.PositionCorrection.z);

            if (m_IXScene != null)
            {
                Vector3 p = m_State.CorrectedPosition;
                float y = m_IXScene.GetSceneY(ref p);
                if (y > p.y - 0.1f)
                {
                    m_State.PositionCorrection = new Vector3(
                        m_State.PositionCorrection.x,
                        m_State.PositionCorrection.y + (y - p.y + 0.1f),
                        m_State.PositionCorrection.z);
                }
            }
            Transform realFollow = Follow;
            if (!m_State.PositionCorrection.AlmostZero() && realFollow != null)
            {
                _lookatPercent = Mathf.Clamp01(1 - m_State.PositionCorrection.sqrMagnitude / 25f);
                if (MinYBasedOnFollow != 0.0f)
                {
                    float minY = realFollow.position.y + MinYBasedOnFollow;
                    float nowY = m_State.CorrectedPosition.y;
                    if (nowY < minY)
                        m_State.PositionCorrection += new Vector3(0.0f, minY - nowY, 0.0f);
                }
                if (MaxYBasedOnFollow != 0.0f)
                {
                    float maxY = realFollow.position.y + MaxYBasedOnFollow;
                    float nowY = m_State.CorrectedPosition.y;
                    if (nowY > maxY)
                        m_State.PositionCorrection += new Vector3(0.0f, maxY - nowY, 0.0f);
                }
            }
            else
                _lookatPercent = 1;
            ...
            ICinemachineCamera fromCam, Vector3 worldUp, float deltaTime)
        {
            ...
            if (fromCam != null && m_Transitions.m_InheritPosition)
            {
                ...
                if (inherateY == -999) 
                    ...
                    m_YAxis.Value = inherateY;
                if (fromCam is CinemachineFreeLook)
                {
                    CinemachineFreeLook freelookCam = fromCam as CinemachineFreeLook;
                    transform.position = freelookCam.transform.position;
                    transform.rotation = freelookCam.transform.rotation;
                }
                else if (fromCam is CinemachineMixingCamera)
                {
            ...
            // 在freelook虚拟相机切换完成后 将 继承位置标记置空 防止他干扰相机自旋转
            m_Transitions.m_InheritPosition = false;
            //if (forceUpdate)
            //{
            //    for (int i = 0; i < 3; ++i)
            ...
            for (int i = 0; i < RigNames.Length; ++i)
            {
                ...
                if (src == null)
                {
                    ...
                    if (composer != null)
                    {
                        ...
                        composer.ScreenY = softCenterDefaultsV[i];
        ...
        float UpdateXAxisHeading(CinemachineOrbitalTransposer orbital, float deltaTime, Vector3 up)
        {
            if (this == null)   
                return 0; // deleted
            //if (m_InputValid == false) return CachedXAxisHeading = 0;
        ...
        void PushSettingsToRigs()
        {
            ...
            for (int i = 0; i < m_Rigs.Length; ++i)
            {
                ...
                if (mUseLegacyRigDefinitions)
                {
                    ...
                    m_Orbits[i].Height = mOrbitals[i].m_FollowOffset.y;
                    m_Orbits[i].Radius = -mOrbitals[i].m_FollowOffset.z;
                ...
                mOrbitals[i].m_FollowParam = m_Transitions.m_InheritPosition ? 0 : (m_FollowFlag ? m_FollowParam : 1f);
        ...
        void UpdateCachedSpline()
        {
            ...
            for (int i = 0; i < 3 && cacheIsValid; ++i)
                cacheIsValid = (m_CachedOrbits[i].Height == m_Orbits[i].Height
                    && m_CachedOrbits[i].Radius == m_Orbits[i].Radius);
            if (!cacheIsValid)
            {

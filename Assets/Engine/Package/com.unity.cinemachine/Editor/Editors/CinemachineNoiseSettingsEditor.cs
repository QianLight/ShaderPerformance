    ...
    internal sealed class NoiseSettingsEditor : BaseEditor<NoiseSettings>
    {
            ...
            SerializedProperty property, int channel, GUIContent title)
        {
            ...
            list.drawHeaderCallback = (Rect rect) =>
                {
                    GUIContent steadyLabel = new GUIContent("(non-random wave if checked, use sin if checked)");
        ...
        private void InitializeLabels(SerializedProperty property)
        {
            if (sinLabel == null)
            {
                SerializedProperty p = property.FindPropertyRelative(() => npDef.Sin);
                sinLabel = new GUIContent(p.displayName, p.tooltip) { text = " " };
            }
            if (steadyLabel == null)
            {
        ...
        private void DrawNoiseChannel(Rect rect, SerializedProperty property)
        {
            ...
            Rect r = rect;
            r.height -= vSpace;
            r.width -= (EditorGUIUtility.singleLineHeight + hSpace )* 2;
            r.width /= 2;
            ...
            r.y -= 1;
            r.x += r.width + hSpace; r.width = EditorGUIUtility.singleLineHeight;
            ...
            EditorGUIUtility.labelWidth = hSpace;
            EditorGUI.PropertyField(r, property.FindPropertyRelative(() => npDef.Sin), sinLabel);
        ...
        void ClearComponent(SerializedProperty p)
        {
            ...
            p.FindPropertyRelative(() => npDef.Sin).boolValue = false;
        }

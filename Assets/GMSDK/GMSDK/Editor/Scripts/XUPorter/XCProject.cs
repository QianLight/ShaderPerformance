...
namespace GMSDKUnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		private PBXDictionary<PBXHeadersBuildPhase> _headersBuildPhases;
		...
		public PBXDictionary<PBXHeadersBuildPhase> headersBuildPhases {
			get {
				if( _headersBuildPhases == null ) {
					_headersBuildPhases = new PBXDictionary<PBXHeadersBuildPhase>( _objects );
				}
				return _headersBuildPhases;
			}
		}
		...
		public bool overwriteBuildSetting( string settingName, string newValue, string buildConfigName = "all") {
			...
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
                //Debug.LogError("build config " + buildConfig);
			...
			if( !string.IsNullOrEmpty( fileReference.buildPhase ) && createBuildFiles ) {
				...
				switch( fileReference.buildPhase ) {
					...
					case "PBXHeadersBuildPhase":
						foreach( KeyValuePair<string, PBXHeadersBuildPhase> currentObject in headersBuildPhases ) {
							Debug.Log( "Adding Header Build File" );
							BuildAddFile(fileReference,currentObject,weak);
						}
						break;
		...
		public PBXCopyFilesBuildPhase AddEmbedFrameworkBuildPhase()
		{
			...
			Debug.LogError("出错了，不能调用该方法AddEmbedFrameworkBuildPhase");
			//#if UNITY_IOS
			//PBXNativeTarget naviTarget = GetNativeTarget(UnityEditor.iOS.Xcode.PBXProject.GetUnityTargetName());
			//if (naviTarget == null)
			//{
		...
		private void BuildAddFile (PBXFileReference fileReference, KeyValuePair<string, PBXHeadersBuildPhase> currentObject,bool weak)
		{
			PBXBuildFile buildFile = new PBXBuildFile( fileReference, weak );
			buildFiles.Add( buildFile );
			currentObject.Value.AddBuildFile( buildFile );
		...
		{	
			...
			if (mod.embed_binaries != null)
			{
                ...
                this.overwriteBuildSetting("LD_RUNPATH_SEARCH_PATHS", "$(inherited) @executable_path/Frameworks");//如果没有这句话，运行会崩溃，报 image not found 错误
		...
		public void AddShellScripts(string name, string shellScript, bool append) {
			Debug.LogError("出错了，不能调用该方法AddShellScripts");
			//#if UNITY_IOS
			////shell
			//PBXNativeTarget naviTarget = GetNativeTarget(UnityEditor.iOS.Xcode.PBXProject.GetUnityTargetName());
			//int buildActionMask = this.GetBuildActionMask();
			//PBXShellScriptBuildPhase shell = new PBXShellScriptBuildPhase(buildActionMask);
			//shell.Add ("shellPath", "/bin/sh");
			//shell.Add ("shellScript", shellScript);
			//shell.Add ("name", name);
			//var buildPhases = (ArrayList)naviTarget.data["buildPhases"];
			//if(append) {
			//	buildPhases.Add(shell.guid);//add build phase
			//}
			//else {
			//	buildPhases.Insert(0, shell.guid);
			//}

			//shellScriptBuildPhases.Add(shell);
			//#endif
		}
		...
		public void Consolidate()
		{
			...
			consolidated.Append<PBXHeadersBuildPhase>( this.headersBuildPhases );

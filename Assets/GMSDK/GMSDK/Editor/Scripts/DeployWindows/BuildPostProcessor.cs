...
namespace GSDK.UnityEditor
{
    public class BuildPostProcessor
    {
        ...
        public static void OnPostProcessBuild_CF(BuildTarget target, string pathToBuiltProject)
        {
            ...
            string srcPluginsFolder = string.Format("{0}/{1}", Application.dataPath, "GSDK/Plugins/Windows/x86_64");
            switch (target)
            {
            ...
            string buildPluginsPath = "";
            if (target == BuildTarget.StandaloneWindows)
            {
                buildPluginsPath = string.Format("{0}/{1}_Data/Plugins/x86", buildPath, Path.GetFileNameWithoutExtension(executableName));
            }
            else if (target == BuildTarget.StandaloneWindows64)
            {
                buildPluginsPath = string.Format("{0}/{1}_Data/Plugins/x86_64", buildPath, Path.GetFileNameWithoutExtension(executableName));
            }
#else            

...
namespace XLua
{
    ...
    public class MethodWrapsCache
    {
        ...
        public MethodWrap _GenMethodWrap(Type type, string methodName, MemberInfo[] methodBases, bool forceCheck = false)
        {
            List<OverloadMethodWrap> overloads = new List<OverloadMethodWrap>();
            for (int i = 0; i < methodBases.Length; ++i)
            {
                var mb = methodBases[i] as MethodBase;
                if (mb == null)
                    continue;
        ...
        public MethodWrap _GenMethodWrap(Type type, string methodName, List<MemberInfo> methodBases, bool forceCheck = false)
        {
            List<OverloadMethodWrap> overloads = new List<OverloadMethodWrap>();

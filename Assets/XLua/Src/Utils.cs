	...
	public static partial class Utils
	{
			...
			int obj_field, int obj_getter, int obj_setter, int obj_meta, out LuaCSFunction item_getter, out LuaCSFunction item_setter, BindingFlags access)
		{
            ...
            for (int i = 0; i < fields.Length; ++i)
			{
				...
				if (field.IsStatic && (SimpleTools.StartsWith(field.Name, "__Hotfix") || SimpleTools.StartsWith(field.Name, "_c__Hotfix")) && typeof(Delegate).IsAssignableFrom(field.FieldType))
				{
					continue;
				}
			...
			for (int i = 0; i < methods.Length; ++i)
			{
				...
				if ((SimpleTools.StartsWith(method_name, "add_") || SimpleTools.StartsWith(method_name, "remove_")) && method.IsSpecialName)
				{
					continue;
				}

				if (SimpleTools.StartsWith(method_name, "op_") && method.IsSpecialName) // 操作符
				{
				...
				else if (SimpleTools.StartsWith(method_name, "get_") && method.IsSpecialName && method.GetParameters().Length != 1) // getter of property
				{
				...
				else if (SimpleTools.StartsWith(method_name, "set_") && method.IsSpecialName && method.GetParameters().Length != 2) // setter of property
				{
					string prop_name = method.Name.Substring(4);
					LuaAPI.xlua_pushasciistring(L, prop_name);
					translator.PushFixCSFunction(L, translator.methodWrapsCache._GenMethodWrap(method.DeclaringType, prop_name, new MethodBase[] { method }).Call);
			...
			{
				if (SimpleTools.StartsWith(kv.Key.Name, "op_")) // 操作符
				{
					LuaAPI.xlua_pushasciistring(L, InternalGlobals.supportOp[kv.Key.Name]);
					translator.PushFixCSFunction(L,
						new LuaCSFunction(translator.methodWrapsCache._GenMethodWrap(type, kv.Key.Name, kv.Value).Call));
			...
			try
			{
				...
				switch (memberType)
				{
					case LazyMemberTypes.Method:
						...
						wrap = new LuaCSFunction(translator.methodWrapsCache._GenMethodWrap(type, memberName, methods).Call);
						if (isStatic)
						{
							LoadCSTable(L, type);
						}

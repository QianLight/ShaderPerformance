...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        void addAssemblieByName(IEnumerable<Assembly> assemblies_usorted, string name)
        {
            ...
            {
                if (SimpleTools.StartsWith(assemblie.FullName, name) && !assemblies.Contains(assemblie))
                {
        ...
        Delegate getDelegate(DelegateBridgeBase bridge, Type delegateType)
        {
            ...
            var methods = bridge.GetType().GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Where(m => !m.IsGenericMethodDefinition && (SimpleTools.StartsWith(m.Name, "__Gen_Delegate_Imp") || m.Name == "Action")).ToArray();

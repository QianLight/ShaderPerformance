...
{
    [AddComponentMenu("")]
    public partial class RuntimeManager : MonoBehaviour
    {
        ...
        public static void OpenFmodFunction()
        {
            m_enable = true;
        }

        public static void CloseFmodFunction()
        {
            m_enable = false;
        }

        public static bool GetFmodFunction()
        {
            return m_enable;
        }
        ...
        public static RuntimeManager Instance
        {
            get
            {
                //Debug.Log("RuntimeManager start");
                if (initException != null)
                {
                    //Debug.Log("RuntimeManager " + initException);
                    throw initException;
                }
                //Debug.Log("RuntimeManager " + m_enable);
                if (!m_enable) return null;
                //Debug.Log("RuntimeManager m_enable" + m_enable);
                if (instance == null)
                {
        ...
        void CheckInitResult(FMOD.RESULT result, string cause)
        {
            ...
            {
                //Debug.Log("CheckInitResult " + result.ToString() + "   " + cause);
                ReleaseStudioSystem();
...
retry:
            ...
            int mode = XUpdater.XSDKManager.singleton.GetFmodInitMode();
            FMOD.INITFLAGS initMode = (FMOD.INITFLAGS)(mode);
            result = studioSystem.initialize(virtualChannels, studioInitFlags, initMode, IntPtr.Zero);
            Debug.Log("fmod init mode=" + mode + "  " + initMode);
        ...
        void Update()
        {
            if (!m_enable || instance == null) return;
        ...
        public void OnApplicationPauseForAllPlatform(bool pauseStatus)
        {
            if (!m_enable || instance == null) return;
        ...
        {
            //Debug.Log("LoadBank start " + bankName);
            ...
            else
            {
                ...
                string persistentBankFolder = Application.persistentDataPath + "/OuterPackage";
                ...
                string bankPath = string.Format("{0}/{1}{2}", persistentBankFolder, bankName, bankName.EndsWith(BankExtension) ? string.Empty : BankExtension);
                if (!File.Exists(bankPath))
                {
                    bankPath = string.Format("{0}/{1}{2}", bankFolder, bankName, bankName.EndsWith(BankExtension) ? string.Empty : BankExtension);
                }


                //Debug.Log("LoadBank path= " + bankPath);


#if UNITY_ANDROID && !UNITY_EDITOR
                if (Settings.Instance.AndroidUseOBB)
                {
                    //Debug.Log("LoadBank Settings.Instance.AndroidUseOBB");
#if UNITY_2018_1_OR_NEWER
                    //Debug.Log("LoadBank UNITY_2018_1_OR_NEWER");
                    Instance.StartCoroutine(Instance.loadFromWeb(bankPath, bankName, loadSamples));
#else
                    //Debug.Log("LoadBank www");
...
#endif // (UNITY_ANDROID || UNITY_WEBGL) && !UNITY_EDITOR
                {
                    ...
                    //Debug.Log("LoadBank loadResult=" + loadResult.ToString());
                    Instance.loadedBankRegister(loadedBank, bankPath, bankName, loadSamples, loadResult);
                }
            }
        }
        ...
        {
            //Debug.Log("fmod LoadBanks start");

            if (fmodSettings.ImportType == ImportType.StreamingAssets)
            {
                //Debug.Log("fmod LoadBanks start " + fmodSettings.ImportType + "  " + fmodSettings.AutomaticSampleLoading);
        ...
        private IEnumerable<string> BanksToLoad(Settings fmodSettings)
        {
            ...
            {
                //bankLoadType = BankLoadType.Specified;
            }
            Debug.Log("BanksToLoad bankLoadType=" + bankLoadType);
        ...
        public void UnloadBanks()
        {
            ...
            banks.AddRange(Instance.loadedBanks.Keys);
            foreach (var item in banks)
            {
                UnloadBank(item);
                Debug.Log("UnloadBanks=" + item);
            }
        }
        ...
        public static Guid PathToGUID(string path)
        {
            ...
            //if (path.StartsWith("{"))
            if(CFEngine.SimpleTools.StartsWith(path, "{"))
            {
        ...
        public static bool HasEvent(string path)
        {
            Guid guid = Guid.Empty;
            FMOD.RESULT result = Instance.studioSystem.lookupID(path, out guid);
            return result != FMOD.RESULT.ERR_EVENT_NOTFOUND;
        }
        ...
        public static void SetListenerLocation(int listenerIndex, ref Vector3 forward, ref Vector3 up, ref Vector3 position)
        {
            FMOD.ATTRIBUTES_3D attributes = new FMOD.ATTRIBUTES_3D();
            attributes.forward = forward.ToFMODVector();
            attributes.up = up.ToFMODVector();
            attributes.position = position.ToFMODVector();
            Instance.studioSystem.setListenerAttributes(listenerIndex, attributes);
        }

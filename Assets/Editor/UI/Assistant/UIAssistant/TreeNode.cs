...
public class TreeNode
{
    ...
    public void UpdataInfo() 
    {
        ...
        if (nodeInfo.Use) 
        {
            if (nodeInfo.check > -1) 
            {
                title = string.Format("{0} N:{1}   info : ( D:{2}    M:{3}    T:{4}    IM:{5}    I2:{6})       check : {7}      BatchID : {8} )", nodeInfo.HierarychyOrder, nodeInfo.Name, nodeInfo.Depth, nodeInfo.MaterialInstanceName, nodeInfo.TextureName, nodeInfo.IsInMask, nodeInfo.IsInMask2D, nodeInfo.check, nodeInfo.BatchID);
            }
            else 
            {
                title = string.Format("{0} N:{1}   info : ( D:{2}    M:{3}     T:{4}    IM:{5}    I2:{6})       BatchID : {7}", nodeInfo.HierarychyOrder, nodeInfo.Name, nodeInfo.Depth, nodeInfo.MaterialInstanceName, nodeInfo.TextureName, nodeInfo.IsInMask, nodeInfo.IsInMask2D, nodeInfo.BatchID);
            }
        }
        for (int i = 0; i < children.Count; i++)
        {
    ...
    private void GenInfo() 
    {
        ...
        nodeInfo.IsInMask2D = assetObject.GetComponent<CFRectMask2D>() != null ? nodeInfo.GetHashCode() : -1;
        if (ui != null) 
        {
            nodeInfo.MaterialInstanceID = ui.material.GetInstanceID();
            nodeInfo.MaterialInstanceName = ui.material.name;
            nodeInfo.TextureID = ui.mainTexture.GetInstanceID();
            nodeInfo.TextureName = ui.mainTexture.name;
        }
        Matrix4x4 localToWorldMatrix = assetObject.transform.localToWorldMatrix;
        CFText t = assetObject.GetComponent<CFText>();
        if(t != null) 
        {
            if (t.text.Equals(""))
            {
        ...
        {
            if(rectTransform != null) rectTransform.GetWorldCorners(nodeInfo.Corners);
        }
    }
}
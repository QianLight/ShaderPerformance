...
[CustomEditor(typeof(QteClickSignal))]
public class QteClickSignalEditor : Editor
{
    private QteClickSignal m_signal;
    private SerializedObject m_so;

    private SerializedProperty m_time;

    public SerializedProperty m_enable;                     //是否开启
    public SerializedProperty m_prefabName;                 //使用的prefab
    public SerializedProperty m_position;                   //UI的位置
    public SerializedProperty m_qteType;                    //QTE的类型
    public SerializedProperty m_isLoop;                     //单击QTE，是否循环
    ...
    private void OnEnable()
    {
        ...
        m_failEvent = m_so.FindProperty("m_failEvent");
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (!m_signal.m_isLoop) //对于不是循环的qte，这些属性才绘制
        {
            EditorGUILayout.PropertyField(m_failEvent, true);
        }

...
namespace OPProject.ProjectFolderAsset
{
    [CanEditMultipleObjects, CustomEditor(typeof(DefaultAsset))]
    public class FileNote : UnityEditor.Editor
    {
        ...
        private void DrawFolder()
        {
            ...
            //EditorGUILayout.LabelField("Comment", new GUIStyle(){fontSize = 20});

            GUI.changed = false;
            _comment = EditorGUILayout.TextArea(_comment, GUILayout.MinHeight(20));
            if (GUI.changed)
            {
    ...
    public class ProjectFolderAssetDataManager
    {
        ...
        private string SAVE_DATA_PATH
        {
            get
            {
                return Application.dataPath + "/../ProjectSettings/OPProjectProjectFolderAssetData.txt";
            }
        }
        ...
        static private void ProjectWindowItemOnGUI(string guid, Rect selectionRect)
        {
            ...
            if (null == _guiStyleLabelTree)
            {
                ...
                _guiStyleLabelNotTree.fontSize = 12;
            }

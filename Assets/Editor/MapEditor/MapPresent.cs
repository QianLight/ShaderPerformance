	...
	class MapPresent
	{
        ...
        private void DrawGridMesh(int gz, int gx)
        {
            ...
            for (int z = 0; z < BlockSize; z++)
            {
                ...
                {
                    if (z + BlockSize * gz < _generator._row && x + BlockSize * gx < _generator._col)
                    {
                        int debug = _generator._col * BlockSize * gz + _generator._col * z + gx * BlockSize + x;
                        float height = _generator.GetHeight(debug);

                        float pos_x = _generator._min.x + gx * BlockSize * _generator._grid_size + x * _generator._grid_size;
                        float pos_y = _generator._min.y + height + 0.02f;
                        float pos_z = _generator._min.z + gz * BlockSize * _generator._grid_size + z * _generator._grid_size;

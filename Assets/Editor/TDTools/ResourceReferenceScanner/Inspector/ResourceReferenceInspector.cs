...
namespace TDTools.ResourceScanner {
    ...
    public class ResourceReferenceInspector : EditorWindow {
        ...
        [MenuItem("Tools/TDTools/���ü�鹤��/���ÿ��ӻ�����")]
        ...
        void CreateBoard() {
            ...
            Label labelSearchBar = new Label("ɸѡ");
            ...
            VisualElement MakeItem(){ 
                ...
                label.RegisterCallback<ContextClickEvent>(obj => {
                    ...
                    if (!filtered[index].IsFavorite) {
                        menu.AddItem(new GUIContent($"�ղ�: {filtered[index].ID}"), false, () => {
                            favoriatedItems.Add(filtered[index]);
                            OnSearchChange(searchField.value);
                        });
                    } else {
                        menu.AddItem(new GUIContent($"ɾ���ղ�: {filtered[index].ID}"), false, () => {
                            favoriatedItems.Remove(filtered[index].type, filtered[index].ID);
                            OnSearchChange(searchField.value);
                        });
                    }
                    menu.AddItem(new GUIContent($"����: {filtered[index].ID}"), false, () => { GUIUtility.systemCopyBuffer = filtered[index].ID; });

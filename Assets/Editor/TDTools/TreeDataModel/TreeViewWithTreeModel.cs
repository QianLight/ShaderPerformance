...
namespace TDTools
{
	...
	internal class TreeViewWithTreeModel<T> : TreeView where T : TreeElement
	{
		...
		protected void CenterRectUsingComplexHeight(ref Rect rect, int lines)
		{
			float linesHeight = EditorGUIUtility.singleLineHeight * lines;
			if (rect.height > linesHeight)
			{
				rect.y += (rect.height - linesHeight) * 0.5f;
				rect.height = linesHeight;
			}
		}
	}

...
namespace TDTools {
    public class PrefabBoneScanner : EditorWindow {
        ...
        [MenuItem("Tools/TDTools/废弃工具/Prefab骨骼扫描工具")]
        ...
        public void CreateGUI() {
            ...
            titleContent = new GUIContent("Prefab骨骼扫描工具");
            ...
            buttonSave.text = "保存结果";
            buttonSave.clicked += () => {
                string path = EditorUtility.SaveFilePanel("保存结果", "", "骨骼扫描结果", "txt");
        ...
        void Scan() {
            ...
            var boneTable = TableChecker.TableChecker.ReadTable(EditorUtility.OpenFilePanel("打开表格", "", "txt"), true);
            ...
            for (int i = 0; i < table.Count; i++) {
                ...
                if (dic.ContainsKey(row["Prefab"])) {
                    ...
                    for (int j = 0; j < list2.Count; j++) {
                        ...
                        if (child == null) {
                            results.Add($"已配找不到\t{row["Prefab"]}\t{list2[j].Value}");
                        } else {
                            keys.Add(list2[j].Key);
                            results.Add($"已配\t{row["Prefab"]}\t{list2[j].Value}");
                        }
                    }
                }
                ...
                for (int j = 0; j < list.Count; j++) {
                    ...
                    if (child == null) {
                        result1.Add($"默认1找不到\t{row["Prefab"]}\t{list[j].Value}");
                    } else {
                        found1.Add($"默认1\t{row["Prefab"]}\t{list[j].Value}");
                    }
                }
                ...
                for (int j = 0; j < list.Count; j++) {
                    ...
                    values.Add(list[j].Value);
                    if (child == null) {
                        result2.Add($"默认2找不到\t{row["Prefab"]}\t{list[j].Value}");
                    } else {
                        found2.Add($"默认2\t{row["Prefab"]}\t{list[j].Value}");
                    }
                }

...
namespace TDTools {
    public class SkillBuffScanningNode : EditorWindow {
        ...
        [MenuItem("Tools/TDTools/废弃工具/技能buff节点扫描")]
        public async static void Run() {
            ...
            await Task.Run(() => {
                ...
                for (int i = 0; i < monsterTable.Table.Count; i++) {
                    try {
                        ...
                        if (data.BuffData.Count >= 2) {
                            bool has22 = false;
                            bool hass23 = false;
                            
                            for (int j = 0; j < data.BuffData.Count; j++) {
                                if (data.BuffData[j].BuffID == 22)
                                    has22 = true;
                                if (data.BuffData[j].BuffID == 23)
                                    hass23 = true;

                            }

                            if (has22 && hass23) {
                                Debug.Log($"技能<color=green>{row["SkillScript"]}</color>有PreCondition节点");
            ...
            using StreamWriter sw = new StreamWriter(EditorUtility.SaveFilePanel("保存", "", "buff结果", "txt"));

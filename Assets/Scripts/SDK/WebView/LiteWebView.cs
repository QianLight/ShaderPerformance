...
namespace LiteWebView
{
    ...
    public class WebView : MonoBehaviour
    {
        ...
        public void Init()
        {
            if (null == _ulite)
            {
                DebugLog.AddLog2("LiteWebView init");

#if !UNITY_EDITOR
#if UNITY_ANDROID
                DebugLog.AddLog2 ("LiteWebView:Android");
                _ulite = new LiteAndroidWebView (getFullName (this.gameObject));
#elif UNITY_IOS
                DebugLog.AddLog2 ("LiteWebView:iOS");
        ...
        {
            DebugLog.AddLog2("loading url: {0}  ID：{1}", url, this.GetInstanceID());
            onLoadingUrl?.Invoke(url);
        }

        void OnJsCall(string msg)
        {
            DebugLog.AddLog2("js call unity: " + msg);
            string iName = null;
            ...
            {
                DebugLog.AddErrorLog2("LiteWebView：Wrong JS Msg [{0}],ex:{1}", msg, e.Message);
            }
        }

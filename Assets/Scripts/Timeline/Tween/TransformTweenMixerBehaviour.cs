...
public class TransformTweenMixerBehaviour : PlayableBehaviour
{
    ...
    public override void ProcessFrame(Playable playable, FrameData info, object playerData)
    {
        ...
        if (trackBinding == null && go != null) trackBinding = go.transform;
        ...
        for (int i = 0; i < inputCount; i++)
        {
            ...
            double curTime = playableInput.GetTime();

            if (Application.isPlaying && curTime > 0 && input.ignorePause && !input.alreadyTweenerDone)
            {
                RTimeline.singleton.startTweenerCountDown = true;
                curTime += RTimeline.singleton.tweenerTimer;
            }

            float normalisedTime = (float)(curTime / playableInput.GetDuration());
            float tweenProgress = input.EvaluateCurrentCurve(normalisedTime);

            if(Application.isPlaying && input.ignorePause && !input.alreadyTweenerDone && tweenProgress == 1)
            {
                RTimeline.singleton.startTweenerCountDown = false;
                RTimeline.singleton.tweenerTimer = 0;
                input.alreadyTweenerDone = true;
            }

            if (input.alreadyTweenerDone)
                tweenProgress = 1;

...
namespace UnityEditor.Recorder
{
    public class RecorderWindow : EditorWindow
    {
        ...
        private string s_FileOutDir = "";

        [MenuItem(MenuRoot + "Recorder Window &R", false, MenuRootIndex)]
        ...
        void CreateView()
        {
            ...
            var sheet2 = Resources.Load<StyleSheet>(s_StylesFolder +
                (EditorGUIUtility.isProSkin ? "recorder_darkSkin" : "recorder_lightSkin"));
            bool sheetNotFound = sheet1 == null || sheet2 == null;
            s_FileOutDir = EditorPrefs.GetString("PREF_RECD", RecorderSettings.OutputDir);
            if (sheetNotFound)
            {
                s_NeedToResetView = true;
                return;
            ...
            m_RecordModeOptionsPanel = new IMGUIContainer(() =>
            {
                ...
                EditorGUILayout.LabelField("录制目录：", s_FileOutDir);
                if (GUILayout.Button("...", GUILayout.Width(20)))
                {
                    s_FileOutDir = EditorUtility.OpenFolderPanel("选择要录制的目录", s_FileOutDir, "");
                    if (!string.IsNullOrEmpty(s_FileOutDir))
                    {
                        EditorPrefs.SetString("PREF_RECD", s_FileOutDir);
                        RecorderSettings.OutputDir = s_FileOutDir;
                    }
                }
            ...
            m_RecordingListItem = new RecorderItemList { name = "recorderList" };
        ...
        void OnEnable()
        {
            ...
            {
                s_Request = UnityEditor.PackageManager.Client.List();
                EditorApplication.update += Progress;
            }
            CreateView();
            RegisterCallbacks();
            CFAudioRecorder.InitRecordAudio();
        }
        ...
        {
            if (m_RecordingListItem == null) return;

            if (m_RecordingListItem.items == null || m_RecordingListItem.items.Count == 0) return;

            if (!m_RecordingListItem.HasFocus()) return;
        ...
        string CheckRecordersIncompatibility()
        {
            ...
            {
                var path = recorder.BuildAbsolutePath();
        ...
        void StartRecordingInternal()
        {
            ...
            CFAudioRecorder.StartRecordAudio();
            ...
            else
            {
                StopRecordingInternal();
                m_State = State.Error;
                CFAudioRecorder.StopRecordAudio();
            }
        ...
        {
            m_RecordButton.text = m_State == State.Recording ? "STOP RECORDING(F10)" : "START RECORDING(F10)";
        }
            ...
            if (m_SelectedRecorderItem != null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        EditorGUILayout.LabelField("Recorder Type", ObjectNames.NicifyVariableName(editor.target.GetType().Name));
        ...
        void OnDestroy()
        {
            ...
            CFAudioRecorder.DestoryRecordAudio();
        }
        ...
        {
            return ObjectNames.GetUniqueName(m_ControllerSettings.RecorderSettings.Select(r => r.name).ToArray(), desiredName);
        }
        ...
        void OnRecordContextMenu(RecorderItem recorder)
        {
            ...
            else
            {
                contextMenu.AddItem(Styles.DuplicateLabel, false,
                    data => { DuplicateRecorder((RecorderItem)data); }, recorder);

                contextMenu.AddItem(Styles.DeleteLabel, false,
                    data => { DeleteRecorder((RecorderItem)data, true); }, recorder);
            }

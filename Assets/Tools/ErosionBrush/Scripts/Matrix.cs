	...
	public class Matrix : Matrix2<float>
	{
		...
		#region Resize
			...
			public float GetInterpolated (float x, float z)
			{
				...
				if (px < rect.offset.x) px = rect.offset.x;
				int nx = px+1; if (nx >= rect.offset.x+rect.size.x) nx = rect.offset.x+rect.size.x-1;

				int pz = (int)z; if (z<0) pz--; if (pz < rect.offset.z) pz = rect.offset.z;
				int nz = pz+1; if (nz >= rect.offset.z+rect.size.z) nz = rect.offset.z+rect.size.z-1;

				float percentX = x-px;
				float percentZ = z-pz;

				int pos_fxfz = (pz-rect.offset.z)*rect.size.x + px - rect.offset.x;
				float val_pxpz = array[pos_fxfz];
				float val_nxpz = array[(pz-rect.offset.z)*rect.size.x + nx - rect.offset.x]; //array[pos_fxfz + 1]; //do not use fast calculations as they are not bounds safe
				float val_pxnz = array[(nz-rect.offset.z)*rect.size.x + px - rect.offset.x]; //array[pos_fxfz + rect.size.z];
				float val_nxnz = array[(nz-rect.offset.z)*rect.size.x + nx - rect.offset.x]; //array[pos_fxfz + rect.size.z + 1];

...
using UnityEngine; //to copy properties
...
namespace ErosionBrushPlugin
{
	static public class Extensions
	{
		...
		{
			Vector2 r1min = r1.min;
			Vector2 r1max = r1.max;
			Vector2 r2min = r2.min;
			Vector2 r2max = r2.max;
		...
		{
			int posX = (int) (pos.x + 0.5f);
			if (pos.x < 0) posX--; //snippet for RoundToInt
			int posZ = (int) (pos.y + 0.5f);
			if (pos.y < 0) posZ--;
		...
		public static CoordRect ToCoordRect (this Vector3 pos, float range, float cellSizeX, float cellSizeZ) //this one works with Terrain Sculptor
		{
			Coord min = new Coord (Mathf.FloorToInt ((pos.x - range) / cellSizeX), Mathf.FloorToInt ((pos.z - range) / cellSizeZ));
			Coord max = new Coord (Mathf.FloorToInt ((pos.x + range) / cellSizeX), Mathf.FloorToInt ((pos.z + range) / cellSizeZ)) + 1;
			return new CoordRect (min, max - min);
		}
		...
		public static CoordRect GetHeightRect (this Terrain terrain)
		{
			...
			int posX = (int) (terrain.transform.localPosition.x / pixelSize + 0.5f);
			if (terrain.transform.localPosition.x < 0) posX--;
			int posZ = (int) (terrain.transform.localPosition.z / pixelSize + 0.5f);
			if (terrain.transform.localPosition.z < 0) posZ--;
		...
		{
			if (offsetX < 0) { sizeX += offsetX; offsetX = 0; }
			if (offsetZ < 0) { sizeZ += offsetZ; offsetZ = 0; } //Not Tested!
			int res = data.heightmapResolution;
			if (sizeX + offsetX > res) sizeX = res - offsetX;
			if (sizeZ + offsetZ > res) sizeZ = res - offsetZ;
...
#if UNITY_EDITOR
#if !UNITY_5_5
			UnityEditor.EditorSelectedRenderState state = show?UnityEditor.EditorSelectedRenderState.Wireframe : UnityEditor.EditorSelectedRenderState.Hidden;
			UnityEditor.EditorUtility.SetSelectedRenderState (tfm.GetComponent<Renderer> (), state);
		...
		{
			if (dict.ContainsKey (key)) { if (replace) dict[key] = value; }
		...
		public static void Normalize (this float[, , ] array, int pinnedLayer)
		{
			...
			int maxZ = array.GetLength (1);
			int numLayers = array.GetLength (2);

...
namespace CFEngine.ShaderVariantsStripper
{
    ...
    {
        public bool IsSameShaderLength = false;
        ...
        public bool Completion(Shader shader, ShaderVariantsData data)
        {
            ...
            if (!IsSameShaderLength)
                return combinationValue == keywords.Count && keywords.Count != 0;
            else
            {
                return combinationValue == keywords.Count && keywords.Count != 0 &&
                       combinationValue == data.shaderKeywordSet.GetShaderKeywords().Length;
            }
        }
        ...
        public void OnGUI(ShaderVariantsStripperConditionOnGUIContext context)
        {
            ...
            IsSameShaderLength = EditorGUILayout.Toggle("组合长度是否需要相同", IsSameShaderLength);
        ...
        public string GetName() => "包含Keyword或集合(包含其中一个关键字)";
#endif
    }

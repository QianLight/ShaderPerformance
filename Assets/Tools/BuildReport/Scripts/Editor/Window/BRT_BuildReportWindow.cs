...
public class BRT_BuildReportWindow : EditorWindow
{
	...
	void _OpenBuildInfo(string filepath)
	{
		...
		Debug.Log("_OpenBuildInfo:"+filepath);
	...
	void OnGUI()
	{
		...
		if (GUILayout.Toggle(IsInOverviewCategory, Labels.OVERVIEW_CATEGORY_LABEL, leftTabStyle, LayoutExpandWidth))
		{
			_selectedCategoryIdx = OVERVIEW_IDX;
		}

		if (GUILayout.Toggle(IsInBuildSettingsCategory, Labels.BUILD_SETTINGS_CATEGORY_LABEL, midTabStyle, LayoutExpandWidth))
		{
			_selectedCategoryIdx = BUILD_SETTINGS_IDX;
		}

		if (_unityBuildReport != null && GUILayout.Toggle(IsInBuildStepsCategory, "构建过程", midTabStyle, LayoutExpandWidth))
		{
			_selectedCategoryIdx = BUILD_STEPS_IDX;
		}

		if (GUILayout.Toggle(IsInSizeStatsCategory, "大小统计信息", midTabStyle, LayoutExpandWidth))
		{
			_selectedCategoryIdx = SIZE_STATS_IDX;
		}

		if (GUILayout.Toggle(IsInUsedAssetsCategory, "使用的资产", midTabStyle, LayoutExpandWidth))
		{
		...
		if (GUILayout.Toggle(IsInUnusedAssetsCategory, "未使用的资产", rightTabStyle, LayoutExpandWidth))
		{

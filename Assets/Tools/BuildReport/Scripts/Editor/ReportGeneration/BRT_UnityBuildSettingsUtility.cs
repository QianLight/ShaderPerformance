...
namespace BuildReportTool
{
	public static class UnityBuildSettingsUtility
	{
		...
		public static void PopulateGeneralSettings(UnityBuildSettings settings)
		{
			...
			settings.WinIncludeNativePdbFilesInBuild = false;// UnityEditor.WindowsStandalone.UserBuildSettings.copyPDBFiles;
#else
			settings.WinIncludeNativePdbFilesInBuild = false;
#endif
#if UNITY_2017_1_OR_NEWER
			settings.WinCreateVisualStudioSolution = false;// UnityEditor.WindowsStandalone.UserBuildSettings.createSolution;
#else
 ...
 PlayerSettings.bundleIdentifier; // ("Bundle Identifier" in iOS, "Package Identifier" in Android)
			...
			List<BuildReportTool.UnityBuildSettings.PackageEntry> packageList, List<BuildReportTool.UnityBuildSettings.BuiltInPackageEntry> builtInPackageList)
		{
			...
			var manifest = BRT_BuildReport_MiniJSON.Json.Deserialize(manifestJsonText) as Dictionary<string, object>;
			if (manifest == null)
			{
				return;
			}
			...
			{
				var locks = BRT_BuildReport_MiniJSON.Json.Deserialize(packagesLockJsonText) as Dictionary<string, object>;
					...
					foreach (var pair in dependencies)
					{
						...
						if (!string.IsNullOrEmpty(newEntry.VersionUsed))
						{
							...
							if (!string.IsNullOrEmpty(newEntry.LocalPath))
							{
								...
								{
									var packageManifest = BRT_BuildReport_MiniJSON.Json.Deserialize(System.IO.File.ReadAllText(packageManifestPath)) as Dictionary<string, object>;

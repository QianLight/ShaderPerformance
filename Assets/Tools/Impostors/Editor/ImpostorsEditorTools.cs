    ...
    public static class ImpostorsEditorTools
    {
        ...
        public static void CreateSceneManagers()
        {
            ...
            if (impostorLodGroupManager == null)
            {
                ...
                {
                    impostorLodGroupManager.transform.SetParent(Selection.activeGameObject.transform);
                }
        ...
        public static void CreatePerCameraManagers()
        {
            ...
            {
                if (camera.GetComponent<ImpostorableObjectsManager>())
                {
                    continue;
                }
        ...
        public static GameObject CreateManagerForCamera(Camera camera, Transform parent)
        {
            ...
            go.SetActive(false);
            var impostorableObjectsManager = go.AddComponent<ImpostorableObjectsManager>();
            impostorableObjectsManager.transform.SetParent(parent);
            var type = typeof(ImpostorableObjectsManager);
        ...
        [MenuItem("Tools/Impostors/RefreshImpostorRenders")]
        public static void RefreshImpostorRenders()
        {
            GameObject tempTree = Selection.activeGameObject;
            if (!tempTree)
            {
                Debug.LogError("û��ѡ����Ч�ĸ��ڵ�!");
                return;
            }
            ...
            {
                trees.transform.GetChild(i).GetComponent<ImpostorLODGroup>().LODs[1].renderers = trees.transform.GetChild(i).GetComponent<LODGroup>().GetLODs().Last().renderers;
            }
        }

        public static void SetupImpostorLODGroupToObject(LODGroup lodGroup)
        {
            bool lastState = lodGroup.gameObject.activeInHierarchy;
            ...
            lodGroup.gameObject.SetActive(lastState);
        }

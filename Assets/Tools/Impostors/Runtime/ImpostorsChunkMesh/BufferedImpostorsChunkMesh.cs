    ...
    public class BufferedImpostorsChunkMesh : IImpostorsChunkMesh
    {
        ...
        public Mesh GetMesh()
        {
            if (_isVertexDataBufferChanged)
            {
                ...
                int nLength = _vertexDataBuffer.Length;
                _mesh.SetVertexBufferParams(nLength, MeshLayout);
                _mesh.SetVertexBufferData(_vertexDataBuffer, 0, 0, nLength, 0, MeshUpdateFlags);
                Profiler.EndSample();
        ...
        [BurstCompile]
        private struct JobFillVertexDataBuffer : IJobFor
        {
            [ReadOnly] public NativeArray<Impostor> impostors;
        ...
        private struct JobCalculateBounds : IJob
        {
            ...
            public void Execute()
            {
                ...
                int nStart = counter.Count*4;
                int nEnd = vertexDataBuffer.Length;
                for (int i = nStart; i < nEnd; i++)
                {
                    VertexData vertexData;
                    vertexData.Position = Vector3.zero;
                    vertexData.Normal = Vector3.zero;
                    vertexData.TexCoord0 = float4.zero;
                    vertexData.Color = float4.zero;
                    vertexDataBuffer[i] = vertexData;
                }
            }
        }

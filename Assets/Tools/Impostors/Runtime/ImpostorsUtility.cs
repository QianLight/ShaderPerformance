...
namespace Impostors
{
    public static class ImpostorsUtility
    {
        ...
        {
            return v.magnitude;
            //return Mathf.Max(new Vector2(v.x, v.y).magnitude,
            //    Mathf.Max(new Vector2(v.x, v.z).magnitude, new Vector2(v.y, v.z).magnitude));
        }


        public static float MaxRatio(Vector3 v)
        {
            float fxy = Mathf.Max(Mathf.Abs(v.x / v.y), Mathf.Abs(v.y / v.x));
            float fxz = Mathf.Max(Mathf.Abs(v.x / v.z), Mathf.Abs(v.z / v.x));
            float fyz = Mathf.Max(Mathf.Abs(v.y / v.z), Mathf.Abs(v.z / v.y));

            return Mathf.Max(fxy, Mathf.Max(fxz, fyz));
        }
        ...
        public static class LightProbsUtility
        {
            ...
            public static List<SphericalHarmonicsL2> GetLightProbes(List<int> ids,
                NativeList<ImpostorableObject> impostorableObjects)
            {
                ...
                {
                    var impostorableObject = impostorableObjects[ids[i]];
                    Positions.Add(impostorableObject.data.position);
                }

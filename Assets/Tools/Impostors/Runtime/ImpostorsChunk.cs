...
namespace Impostors
{
    ...
    public class ImpostorsChunk : IDisposable, IMemoryConsumer
    {
            ...
            CompositeRenderTexturePool renderTexturePool, MaterialObjectPool materialObjectPool)
        {
            ...
            _material.SetFloat("_ImpostorAlpha", ImpostorLODGroupsManager.Instance._cutoutTransparentFill);
        ...
        public int GetPlace(ImpostorableObject impostorableObject)
        {
            ...
            switch (requiredAction)
            {
                case ImpostorableObject.RequiredAction.UpdateImpostorTexture:
                    if (impostorableObject.visibleState == ImpostorableObject.VisibilityState.BecameVisible)
                    {
                        impostor.fadeTime = 0; 
                    }
                    else
        ...
        [BurstCompile]
        private struct RemoveAllImpostorsWithImpostorLODGroupInstanceIdJob : IJobFor
        {
            public NativeArray<Impostor> impostors;

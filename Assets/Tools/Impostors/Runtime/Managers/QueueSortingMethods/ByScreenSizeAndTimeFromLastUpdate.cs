...
namespace Impostors.Managers.QueueSortingMethods
{
    public class ByScreenSizeAndTimeFromLastUpdate //: IQueueSortingMethod
    {
        ...
        {
            [ReadOnly] public NativeArray<ImpostorableObject> impostorableObjects;
            public NativeList<IndexAndValue> buffer;
            public NativeQueue<int> queue;
            public int maxUpdates;
            public int maxBackgroundUpdates;
            public float time;
            //public int isOverMemory;
            //public int memoryMinResolution;
            ...
            private void Do(int isVisible, int count, int startBufferIndex)
            {
                ...
                for (int i = 0; i < impostorableObjects.Length; i++)
                {
                    var instanceData = impostorableObjects[i];
                    if (instanceData.visibleState == ImpostorableObject.VisibilityState.NotSet)
                        continue;
                    if (isVisible == 0 && (instanceData.visibleState == ImpostorableObject.VisibilityState.Visible ||
                                           instanceData.visibleState == ImpostorableObject.VisibilityState.BecameVisible))
                        continue;
                    if (isVisible == 1 && (instanceData.visibleState == ImpostorableObject.VisibilityState.Invisible ||
                                           instanceData.visibleState == ImpostorableObject.VisibilityState.BecameInvisible))
                        continue;
                    if (instanceData.requiredAction != ImpostorableObject.RequiredAction.UpdateImpostorTexture)
                        continue;

                    float value = impostorableObjects[i].nowScreenSize *
                                  (time - impostorableObjects[i].lastUpdate.time);
        ...
        public static JobHandle  Sort(NativeArray<ImpostorableObject> impostorableObjects, NativeQueue<int> queue, int maxUpdates,
            int maxBackgroundUpdates,float time, JobHandle jobHandle)
        {
            ...
            var job = new Job()
            {
                ...
                //isOverMemory=isOverMemory,
                //memoryMinResolution=memoryMinResolution,

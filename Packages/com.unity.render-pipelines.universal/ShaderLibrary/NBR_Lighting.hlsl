...
{
    Light light = GetMainLight();
    light.shadowAttenuation = MainLightShadow(shadowCoord, positionWS, shadowMask, _MainLightOcclusionProbes,0);
    return light;
}
...
#endif
    light.shadowAttenuation = AdditionalLightShadow(perObjectLightIndex, positionWS, shadowMask, occlusionProbeChannels,0);
...
inline void InitializeBRDFData(half3 albedo, half metallic, half3 specular, half smoothness, inout half alpha, out BRDFData outBRDFData)
{
    ...
    half3 brdfDiffuse = albedo * oneMinusReflectivity * _IBLScale;
#else
    half3 brdfDiffuse = albedo * oneMinusReflectivity;
#endif

#if defined(_SPECULAR_EXTERNAL)
    half3 brdfSpecular = specular;
#else
...
half3 GlossyEnvironmentReflection(half3 reflectVector, half perceptualRoughness, half occlusion)
{
    ...
    return environmentColor * gradient * occlusion;
#else

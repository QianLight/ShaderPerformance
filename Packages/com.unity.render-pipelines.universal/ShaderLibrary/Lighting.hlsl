...
#include "Assets/Engine/Runtime/Shaders/Shading/Scene/SceneWeather.hlsl"
...
float _SpotLightShadowIntensity;
...
Light GetMainLight(float4 shadowCoord, float3 positionWS, half4 shadowMask, float3 normalWs)
{
    Light light = GetMainLight();
    light.shadowAttenuation = MainLightShadow(shadowCoord, positionWS, shadowMask, _MainLightOcclusionProbes, normalWs);
    return light;
}
...
Light GetAdditionalLight(uint i, float3 positionWS, half4 shadowMask, bool needCloud = true)
{
    int perObjectLightIndex = GetPerObjectLightIndex(i);
...
struct BRDFData
{
    ...
    half darkMetallicMask;
    #endif

    #ifdef _DARK_ENVIROMENT_REFLECT_SCALE_ENABLE
    half darkReflectMask;
    #endif
};
                                     ...
                                     half smoothness, inout half alpha, out BRDFData outBRDFData)
{
    ...
    #ifdef _DARK_METALLIC_IBL_SCALE_ENABLE
    outBRDFData.darkMetallicMask = 0;
    #endif

    #ifdef _DARK_ENVIROMENT_REFLECT_SCALE_ENABLE
    outBRDFData.darkReflectMask = 0;
    #endif
}
                               ...
                               out BRDFData outBRDFData)
{
    ...
    #if defined(_SPECULAR_SCALE_ENABLE)
        brdfSpecular *= _SpecularScale;
    #endif

    #endif

    #ifdef _BASEMAP_SCALE_ENABLE
    brdfDiffuse *= _BaseMapScale;
    #endif

    InitializeBRDFDataDirect(brdfDiffuse, brdfSpecular, reflectivity, oneMinusReflectivity, smoothness, alpha,
                             outBRDFData);
}
...
half3 GetTestReflectColor(half3 reflectVector, half perceptualRoughness, half t)
{
    half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness);
    half4 encodedCurrentIrradiance = SAMPLE_TEXTURECUBE_LOD(_ExplorationCurSpecCube, sampler_ExplorationCurSpecCube, reflectVector, mip);
    half4 encodedTargetIrradiance = SAMPLE_TEXTURECUBE_LOD(_ExplorationTargetSpecCube, sampler_ExplorationTargetSpecCube, reflectVector, mip);
    //TODO:DOTS - we need to port probes to live in c# so we can manage this manually.
    #if defined(UNITY_USE_NATIVE_HDR) || defined(UNITY_DOTS_INSTANCING_ENABLED)
        half3 irradianceCurrent = encodedCurrentIrradiance.rgb;
        half3 irradianceTarget = encodedTargetIrradiance.rgb;
    #else
        half3 irradianceCurrent = DecodeHDREnvironment(encodedCurrentIrradiance, _ExplorationCurSpecCube_HDR);
        half3 irradianceTarget = DecodeHDREnvironment(encodedTargetIrradiance, _ExplorationTargetSpecCube_HDR);
    #endif

    return lerp(irradianceCurrent, irradianceTarget, t);
}

half3 GetEnvironmentColor(half3 reflectVector, half perceptualRoughness)
{
    if (_ExplorationParams.x)
    {
       return GetTestReflectColor(reflectVector, perceptualRoughness, _ExplorationParams.y);
    }
    else
    {
        half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness);
...
{
    half3 finalEnvironmentColor = half3(1, 1, 1);

    #if defined(_ENVIRONMENTREFLECTIONS_OFF)
	float3 environmentColor = float3(0.05,0.07,0.1);	// _GlossyEnvironmentColor.rgb
	float gradient = reflectVector.y + 1.5;	//0.5~2.5
    finalEnvironmentColor = environmentColor * gradient * occlusion;
    
    #else

    half3 irradiance = GetEnvironmentColor(reflectVector, perceptualRoughness);
    finalEnvironmentColor = irradiance * occlusion;
    #endif // GLOSSY_REFLECTIONS

    return finalEnvironmentColor;
}

half3 GlossyEnvironmentReflection(half3 reflectVector, half perceptualRoughness, half occlusion, BRDFData brdfData)
{
    half3 finalEnvironmentColor = half3(1, 1, 1);
    ...
    #ifdef _DARK_ENVIROMENT_REFLECT_SCALE_ENABLE
        finalEnvironmentColor += finalEnvironmentColor * occlusion * _DarkEnviromentRefOffset * brdfData.darkReflectMask;
    #endif
...
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, float clearCoatMask,
                         ...
                         half3 normalWS, half3 viewDirectionWS, half atten)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = saturate(dot(normalWS, viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);
    #if defined(_BAKEGIOFF)
    half3 indirectDiffuse = 1 * occlusion;
    #else
    half3 indirectDiffuse = bakedGI * occlusion;
    #endif
    
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, brdfData.perceptualRoughness, occlusion, brdfData);

    #ifdef _SHADOW_COLOR_ENABLE
	indirectSpecular += (1 - atten) * urp_ShadowColor.rgb * indirectSpecular * 1;
    #endif
    ...
    #endif
    #if defined(_IBLMASK)
    half nov = max(pow(dot(normalWS,viewDirectionWS),_IBLMASKFOAT),_IBLMASKMAX);
    indirectSpecular *= nov;
    #endif
...
half3 LightingPhysicallySpecular(
    ...
    half3 normalWS, half3 viewDirectionWS, half clearCoatMask, bool specularHighlightsOff)
{
    ...
    half3 brdf = brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS);
...
{
    half NdotL = saturate(dot(normalWS, lightDirectionWS));
    half3 radiance = half3(1, 1, 1);

    #ifdef _SHADOW_COLOR_ENABLE
    //增加阴影颜色功能（预览版），直接用系统多余的变量unity_ShadowColor；
    radiance = lightColor * lightAttenuation * NdotL + lightColor * (1.0h - lightAttenuation) * urp_ShadowColor.rgb;
    brdfData.specular *= lightAttenuation;
    #else
    ...
    [branch] if (!specularHighlightsOff)
    {
        half3 brdfSpecular = brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS,
                                                                    viewDirectionWS);

        #if defined(_GLOBAL_SPECULAR_ADJUST_ENABLE)
            brdfSpecular = min(brdfSpecular, _sceneSpecularColorParams.a * 30);
        #endif
...
half3 LightingPhysicallyBasedAddLight(BRDFData brdfData, BRDFData brdfDataClearCoat,
                                      half3 lightColor, half3 lightDirectionWS, half lightAttenuation,
                                      half3 normalWS, half3 viewDirectionWS,
                                      half clearCoatMask, bool specularHighlightsOff)
{
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData)
{
    ...
    Light mainLight = GetMainLight(inputData.shadowCoord, inputData.positionWS, shadowMask, inputData.normalWS);
    ...
    #if defined(_RAIN)
         brdfData.diffuse *= surfaceData.darkValueInRain;
     //    brdfData.specular *= surfaceData.darkValueInRain;
    #endif

    #ifdef _DARK_ENVIROMENT_REFLECT_SCALE_ENABLE
    brdfData.darkReflectMask = (1 - surfaceData.metallic) * (1 - shadowMask.r);
    #endif

    #ifdef _DARK_METALLIC_IBL_SCALE_ENABLE
    brdfData.darkMetallicMask = surfaceData.metallic * (1 - shadowMask.r);
    #endif

    MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI);
    half3 bakeGI = inputData.bakedGI;
    half3 color = GlobalIllumination(brdfData, brdfDataClearCoat, surfaceData.clearCoatMask,
                                     inputData.bakedGI, surfaceData.occlusion,
                                     inputData.normalWS, inputData.viewDirectionWS, mainLight.shadowAttenuation);
    //return half4(color,1);
    //因为计算直接光时漫反射算了阴影颜色，而Specular会乘以atten，导致镜面反射比漫反射暗（金属类的特别明显）
    //与项目组确认后，在计算间接光镜面反射时，把阴影颜色补充上去。
    half3 ColorPBR = LightingPhysicallyBased(brdfData, brdfDataClearCoat,
                                             mainLight.color, mainLight.direction,
                                             mainLight.distanceAttenuation * mainLight.shadowAttenuation * surfaceData.
                                             occlusion,
                                             inputData.normalWS, inputData.viewDirectionWS,
                                             surfaceData.clearCoatMask, specularHighlightsOff);

    #ifdef _BRIGHT_AO_ENABLE
        half rgbAverage = dot(inputData.bakedGI.rgb, half3(1,1,1)) * 0.333;
        half dis = distance(rgbAverage, _BrightAOParams.y);
        half instensity = lerp(0, _BrightAOParams.w, dis);
        instensity = saturate(instensity + _BrightAOParams.z);
        half3 brigtColor = lerp(ColorPBR, ColorPBR * instensity, mainLight.shadowAttenuation);
        color += lerp(ColorPBR, brigtColor, _BrightAOParams.x);
    #else
        color += ColorPBR;
    #endif
    ...
    for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
    {
    ...
    #ifndef SPOT_LIGHT_AS_SPECULAR
            Light light = GetAdditionalLight(lightIndex, inputData.positionWS, 1);
    #if defined(_SCREEN_SPACE_OCCLUSION)
                    light.color *= aoFactor.directAmbientOcclusion;
    #endif
        color += LightingPhysicallyBasedAddLight(brdfData, brdfDataClearCoat,
        ...
        light.color *= aoFactor.directAmbientOcclusion;
    #endif
        color +=  LightingPhysicallySpecular(brdfData, brdfDataClearCoat,
                                     light.color, light.direction, light.distanceAttenuation * light.shadowAttenuation * surfaceData.occlusion,
                                     inputData.normalWS, inputData.viewDirectionWS,
                                     surfaceData.clearCoatMask, specularHighlightsOff)  ;
         half isShadow = saturate((((light.distanceAttenuation) > 0 )* (shadowAttenuation < .9)) );
         half atten = isShadow * shadowAttenuation + (1- isShadow);
    ...
    #endif
    CalcSceneColor(color, mainLight.shadowAttenuation);
    ...
    #if defined(_LIGHTMAPUNENABLE)
    return half4(bakeGI,surfaceData.alpha);
    #else
                                  ...
                                  half3 emission, half alpha)
{
    ...
    half3 diffuseColor = inputData.bakedGI + LightingLambert(attenuatedLightColor, mainLight.direction,
                                                             inputData.normalWS);
    half3 specularColor = LightingSpecular(attenuatedLightColor, mainLight.direction, inputData.normalWS,
                                           inputData.viewDirectionWS, specularGloss, smoothness);
...
half3 Albedo2Diffuse(half metallic, half3 albedo)
{
    half pi = 4;
    half oneMinusDielectricSpec = 0.220916301;
    half kd = oneMinusDielectricSpec - metallic * oneMinusDielectricSpec;

    return kd * pi * albedo;
}

half4 LowLevelLambert(InputData inputData, half3 albedo, half3 emission, half alpha, half metallic)
{
    // To ensure backward compatibility we have to avoid using shadowMask input, as it is not present in older shaders
    #if defined(SHADOWS_SHADOWMASK) && defined(LIGHTMAP_ON)
    half4 shadowMask = inputData.shadowMask;
    ...
    half3 diffuse = Albedo2Diffuse(metallic, albedo);
    half3 finalColor = diffuseColor * diffuse + emission;
    finalColor += metallic * albedo * 0.4;
    CalcSceneColor(finalColor, mainLight.shadowAttenuation);
    return half4(finalColor, alpha);
}

half4 LowLevelBlinnPhong(InputData inputData, half3 albedo, half3 emission, half alpha){
	  // To ensure backward compatibility we have to avoid using shadowMask input, as it is not present in older shaders
    #if defined(SHADOWS_SHADOWMASK) && defined(LIGHTMAP_ON)
    half4 shadowMask = inputData.shadowMask;
    #elif !defined (LIGHTMAP_ON)
    ...
    half3 diffuseColor = inputData.bakedGI + LightingLambert(attenuatedLightColor, mainLight.direction,
                                                             inputData.normalWS);
    half3 diffuse = Albedo2Diffuse(0, albedo);
    half3 finalColor = diffuseColor * diffuse + emission;
    //finalColor += metallic * albedo * 0.4;
	
	half3 specularColor = Luminance(albedo);
	half ndh = saturate(dot(inputData.normalWS,  normalize(inputData.viewDirectionWS + mainLight.direction)));
	finalColor += specularColor * pow(ndh, 10);
...
half4 LightweightFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness,
                                    half3 emission, half alpha)
{

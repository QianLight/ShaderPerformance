    ...
    public class FinalBlitPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.FinalBlit)))
            {
                ...
                /* Add By:  Takeshi
                 * Apply:   This is the final Process of Fixing UI Alpha Gamma (in case of no Final Post)
                 *          make final image return to Linear Color, when the UI images are already blended alpha with the image of 3D render
                 * Note:    Judge the camera if it is used in Reflection Probe Render,
                 *          and the camera will not fix color gamma in case of render a probe.
                 *          because probe image is not a final image, we must unify the color gamma in final Process.
                 */
#if UNITY_EDITOR
                ...
                /* By:Takeshi Apply: Over the Final Process in fix color gamma */ 

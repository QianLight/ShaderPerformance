...
namespace UnityEngine.Rendering.Universal
{
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        public static readonly int _ZTestHSR = Shader.PropertyToID("_ZTestHSR");
        public static readonly int _ZWriteHSR = Shader.PropertyToID("_ZWriteHSR");
        ...
        private static class Profiling
        {
            ...
            public static ProfilingSampler TryGetOrAddCameraSampler(Camera camera)
            {
                ...
                if (!exists)
                {
                    ...
                    ps = new ProfilingSampler(
                        $"{nameof(UniversalRenderPipeline)}.{nameof(RenderSingleCamera)}: {camera.name}");
 ...
 new ProfilingSampler($"{nameof(RenderPipeline)}.{nameof(BeginContextRendering)}");
                public static readonly ProfilingSampler endContextRendering =
 new ProfilingSampler($"{nameof(RenderPipeline)}.{nameof(EndContextRendering)}");
#else
                public static readonly ProfilingSampler beginFrameRendering =
                    new ProfilingSampler($"{nameof(RenderPipeline)}.{nameof(BeginFrameRendering)}");

                public static readonly ProfilingSampler endFrameRendering =
                    new ProfilingSampler($"{nameof(RenderPipeline)}.{nameof(EndFrameRendering)}");
#endif
                public static readonly ProfilingSampler beginCameraRendering =
                    new ProfilingSampler($"{nameof(RenderPipeline)}.{nameof(BeginCameraRendering)}");

                public static readonly ProfilingSampler endCameraRendering =
                    new ProfilingSampler($"{nameof(RenderPipeline)}.{nameof(EndCameraRendering)}");

                const string k_Name = nameof(UniversalRenderPipeline);

                public static readonly ProfilingSampler initializeCameraData =
                    new ProfilingSampler($"{k_Name}.{nameof(InitializeCameraData)}");

                public static readonly ProfilingSampler initializeStackedCameraData =
                    new ProfilingSampler($"{k_Name}.{nameof(InitializeStackedCameraData)}");

                public static readonly ProfilingSampler initializeAdditionalCameraData =
                    new ProfilingSampler($"{k_Name}.{nameof(InitializeAdditionalCameraData)}");

                public static readonly ProfilingSampler initializeRenderingData =
                    new ProfilingSampler($"{k_Name}.{nameof(InitializeRenderingData)}");

                public static readonly ProfilingSampler initializeShadowData =
                    new ProfilingSampler($"{k_Name}.{nameof(InitializeShadowData)}");

                public static readonly ProfilingSampler initializeLightData =
                    new ProfilingSampler($"{k_Name}.{nameof(InitializeLightData)}");

                public static readonly ProfilingSampler getPerObjectLightFlags =
                    new ProfilingSampler($"{k_Name}.{nameof(GetPerObjectLightFlags)}");

                public static readonly ProfilingSampler getMainLightIndex =
                    new ProfilingSampler($"{k_Name}.{nameof(GetMainLightIndex)}");

                public static readonly ProfilingSampler setupPerFrameShaderConstants =
                    new ProfilingSampler($"{k_Name}.{nameof(SetupPerFrameShaderConstants)}");
                ...
                public static class Renderer
                {
                    ...
                    public static readonly ProfilingSampler setupCullingParameters =
                        new ProfilingSampler($"{k_Name}.{nameof(ScriptableRenderer.SetupCullingParameters)}");

                    public static readonly ProfilingSampler setup =
                        new ProfilingSampler($"{k_Name}.{nameof(ScriptableRenderer.Setup)}");
                };
                ...
                public static class Context
                {
                    ...
                    public static readonly ProfilingSampler submit =
                        new ProfilingSampler($"{k_Name}.{nameof(ScriptableRenderContext.Submit)}");
                };
        ...
        public static event Action<ScriptableRenderContext, Camera[]> setupPerFrameData;
        public static event Action<ScriptableRenderContext, Camera> setupPerCameraData;
        ...
        public static bool planarShadowEnabled;
        ...
        private static readonly HashSet<string> msaaScenes = new HashSet<string>() {"OP_zhucheng"};
        ...
        public static int msaaSampleCount
        {
            get
            {
                ...
                //#endif
                //{
                //    return EngineContext.needMSAA
                //        ? asset.uiMsaaSampleCount
                //        : asset.defaultMsaaSampleCount;
                //}

                //#if UNITY_EDITOR
                //string activeSceneName = SceneManager.GetActiveScene().name;
                //return EngineContext.uiCount > 0 || msaaScenes.Contains(activeSceneName)
                //    ? asset.uiMsaaSampleCount
                //    : asset.defaultMsaaSampleCount;
                //#endif
                ...
                //desc.graphicsFormat = Experimental.Rendering.GraphicsFormat.R8G8B8A8_SRGB;
                //desc.sRGB = true;
                //desc.msaaSamples = 8;
                //desc.depthBufferBits = 32;
                if (asset.AADebug)
                {
                    if (asset.Antialiasing == AntialiasingMode.MSAA && SystemInfo.supportsStoreAndResolveAction)
        ...
        public UniversalRenderPipeline(UniversalRenderPipelineAsset asset)
        {
            ...
            bool msaaSampleCountNeedsUpdate = qualitySettingsMsaaSampleCount != msaaSampleCount;
...
#if ENABLE_VR && ENABLE_XR_MODULE
                XRSystem.UpdateMSAALevel(msaaSampleCount);
#endif
            }
...
#endif
        {
...
#endif
            {
                ...
                VFX.VFXManager.PrepareCamera(camera);
                    ...
                    setupPerCameraData?.Invoke(renderContext, camera);
            ...
            using (new ProfilingScope(null, Profiling.Pipeline.endFrameRendering))
            {
                ...
                Shader.DisableKeyword("_CUSTOM_UI_DISTORTION");
                Shader.DisableKeyword("_CUSTOM_DISTORTION");
                if (EngineContext.IsRunning && EngineContext.instance != null)
                {
                    EngineContext.instance.stateflag.SetFlag(EngineContext.SFlag_Distortion, false);
                }
            }
#endif
            #if UNITY_EDITOR
            if (CaptureWindow.captureRenderCounter-- == 0)
                CaptureWindow.OnRenderFinish();
            #endif
        }
            ...
            bool anyPostProcessingEnabled)
        {
            ...
            if (renderer == null)
            {
                Debug.LogWarning(string.Format(
                    "Trying to render {0} with an invalid renderer. Camera rendering will be skipped.", camera.name));
                return;
            }
            ...
            using (new ProfilingScope(cmdScope, sampler)) // Enqueues a "BeginSample" command into the CommandBuffer cmd
            {
                ...
                context.ExecuteCommandBuffer(
                    cmd); // Send all the commands enqueued so far in the CommandBuffer cmd, to the ScriptableRenderContext context
                cmd.Clear();
            ...
            context.ExecuteCommandBuffer(
                cmd); // Sends to ScriptableRenderContext all the commands enqueued since cmd.Clear, i.e the "EndSample" command
        ...
        static void RenderCameraStack(ScriptableRenderContext context, Camera baseCamera)
        {
            ...
            if (cameraStack != null)
            {
                ...
                for (int i = 0; i < cameraStack.Count; ++i)
                {
                    ...
                    if (currCamera.isActiveAndEnabled)
                    {
                        ...
                        {
                            Debug.LogWarning(string.Format(
                                "Stack can only contain Overlay cameras. {0} will skip rendering.", currCamera.name));
                            continue;
                        }
                        ...
                        if (currCameraRendererType != baseCameraRendererType)
                        {
                            ...
                            {
                                Debug.LogWarning(string.Format(
                                    "Only cameras with compatible renderer types can be stacked. {0} will skip rendering",
                                    currCamera.name));
                                continue;
                            }
                        }
            ...
            setupPerCameraData?.Invoke(context, baseCamera);
...
#pragma warning restore 0618
                ...
                if (isStackedRendering)
                {
                    for (int i = 0; i < cameraStack.Count; ++i)
                    {
                        ...
                        if (currCameraData != null)
                        {
                            ...
                            InitializeAdditionalCameraData(currCamera, currCameraData, lastCamera,
                                ref overlayCameraData);
                            setupPerCameraData?.Invoke(context, baseCamera);
#if ENABLE_VR && ENABLE_XR_MODULE
                            if (baseCameraData.xr.enabled)
        ...
        {
            bool isUiCamera = additionalCameraData != null && additionalCameraData.IsUiCamera;
            bool isMainCamera = additionalCameraData != null && additionalCameraData.IsMainCamera;
            bool debug = false;
#if UNITY_EDITOR
            if (UnityEditor.EditorApplication.isPlaying)
            {
                debug = UnityEngine.Rendering.Universal.UniversalRenderPipeline.asset.AADebug;
            }
#endif
            if (isUiCamera)
                return;
            ...
            else if (camera.cameraType == CameraType.SceneView)
            {
                ...
                trigger = mainAdditionalCameraData != null && mainAdditionalCameraData.volumeTrigger != null
                    ? mainAdditionalCameraData.volumeTrigger
                    : trigger;
            }
            //camera.gameObject.CompareTag("MainCamera")
            VolumeManager.instance.Update(trigger, layerMask, debug, isMainCamera);
        }
        ...
        static bool CheckPostProcessForDepth(in CameraData cameraData)
        {
            ...
            DOFParam dof;
            URPDepthOfField.instance.GetValue(out dof);
            if (dof.active/*stack.GetComponent<DepthOfField>().IsActive()*/)
                return true;
        ...
        static void InitializeCameraData(Camera camera, UniversalAdditionalCameraData additionalCameraData,
            bool resolveFinalTarget, out CameraData cameraData)
        {
            ...
            int msaaSamples = 1;
            if (camera.allowMSAA && msaaSampleCount > 1 && rendererSupportsMSAA)
                msaaSamples = (camera.targetTexture != null)
                    ? camera.targetTexture.antiAliasing
                    : msaaSampleCount;
#if ENABLE_VR && ENABLE_XR_MODULE
            ...
            #if UNITY_ANDROID
            float outlineZOffset = 1000;
            #else
            float outlineZOffset = 10000;
            #endif
            Shader.SetGlobalFloat("_OutlineZOffset", outlineZOffset);
        }
        ...
        static void InitializeStackedCameraData(Camera baseCamera,
            UniversalAdditionalCameraData baseAdditionalCameraData, ref CameraData cameraData)
        {
...
#endif
                cameraData.waterLightDir = null;
            }
            else if (baseAdditionalCameraData != null)
            {
                cameraData.volumeLayerMask = baseAdditionalCameraData.volumeLayerMask;
                cameraData.volumeTrigger = baseAdditionalCameraData.volumeTrigger == null
                    ? baseCamera.transform
                    : baseAdditionalCameraData.volumeTrigger;
                cameraData.isStopNaNEnabled = baseAdditionalCameraData.stopNaN && SystemInfo.graphicsShaderLevel >= 35;
                cameraData.isDitheringEnabled = baseAdditionalCameraData.dithering;
                //bool useFXAA = msaaSampleCount == 1 && EngineContext.aaEnabled;
                //cameraData.antialiasing = useFXAA ? AntialiasingMode.FastApproximateAntialiasing : AntialiasingMode.None;
                //cameraData.antialiasingQuality = baseAdditionalCameraData.antialiasingQuality;
#if ENABLE_VR && ENABLE_XR_MODULE
                cameraData.xrRendering = baseAdditionalCameraData.allowXRRendering && m_XRSystem.RefreshXrSdk();
#endif
                cameraData.waterLightDir = baseAdditionalCameraData.waterLightDir;

                cameraData.isMainCamera = baseAdditionalCameraData.IsMainCamera;
                cameraData.isUiCamera = baseAdditionalCameraData.IsUiCamera;
                cameraData.isUiSceneCamera = baseAdditionalCameraData.IsUiSceneCamera;
            }
            else
            {
                cameraData.volumeLayerMask = 1; // "Default"
            ...
            // cameraData.pixelRect = RenderContext.singleton.pixelRect;
            // cameraData.pixelWidth = RenderContext.singleton.pixelWidth;
            // cameraData.pixelHeight = RenderContext.singleton.pixelHeight;
            ...
            // cameraData.renderScale = (Mathf.Abs(1.0f - settings.renderScale) < kRenderScaleThreshold)
            //     ? 1.0f
            //     : settings.renderScale;
            // if (baseAdditionalCameraData != null)
            // {
            //     cameraData.renderScale = baseAdditionalCameraData.renderType == CameraRenderType.Overlay ? 1f : QualitySettingData.current.rtScale;
            // }
            // else
            // {
                // cameraData.renderScale = QualitySettingData.current.rtScale;
            // }
            ...
            if (hasHSRGPU && EngineContext.optimizeRenderState)
            {
                Shader.SetGlobalInt(_ZTestHSR, 4);
                Shader.SetGlobalInt(_ZWriteHSR, 1);
            }
            else
            {
                Shader.SetGlobalInt(_ZTestHSR, 3);
                Shader.SetGlobalInt(_ZWriteHSR, 0);
            }
            //SortingCriteria customSorting = EngineContext.optimizeRenderState && canSkipFrontToBackSorting ? noFrontToBackOpaqueFlags : commonOpaqueFlags;
            SortingCriteria customSorting = EngineContext.optimizeRenderState ? noFrontToBackOpaqueFlags : commonOpaqueFlags;

            cameraData.defaultOpaqueSortFlags = customSorting;
        ...
        static void InitializeAdditionalCameraData(Camera camera, UniversalAdditionalCameraData additionalCameraData,
            bool resolveFinalTarget, ref CameraData cameraData)
        {
            ...
            //Role Shadow
            cameraData.roleShadowScale = settings.roleShadowScale;
            cameraData.roleDepthBias = settings.roleShadowDepthBias;
            cameraData.roleNormalBias = settings.roleShadowNormalBias;
            cameraData.roleShadowResolution = settings.roleShadowResolution;
            cameraData.roleShadowLayerMask = settings.roleShadowLayerMask;
            cameraData.roleShadowEnable = settings.roleShadowEnable;
            cameraData.roleShadowSize = settings.roleShadowSize;
            cameraData.roleShadowFar = settings.roleShadowFar;
            cameraData.roleShadowNear = settings.roleShadowNear;
            ...
            else if (additionalCameraData != null)
            {
                ...
                cameraData.clearDepth = (additionalCameraData.renderType != CameraRenderType.Base)
                    ? additionalCameraData.clearDepth
                    : true;
                ...
                cameraData.isMainCamera = additionalCameraData.IsMainCamera;
                cameraData.isUiCamera = additionalCameraData.IsUiCamera;
                cameraData.isUiSceneCamera = additionalCameraData.IsUiSceneCamera;
            }
            else
            {
                cameraData.renderType = CameraRenderType.Base;
            ...
            {
                cameraData.renderScale = (Application.isPlaying ? settings.renderScale : 1.0f);
            }

            float width = Screen.width * (Application.isPlaying ? settings.renderScale : 1.0f);
            float height = Screen.height * (Application.isPlaying ? settings.renderScale : 1.0f);
            Vector4 screenParams = new Vector4(width, height, 1f / width, 1f / height);
            Shader.SetGlobalVector("_RenderScaledScreenParams", screenParams);
        ...
        static void InitializeRenderingData(UniversalRenderPipelineAsset settings, ref CameraData cameraData,
            ...
            bool anyPostProcessingEnabled, out RenderingData renderingData)
        {
            ...
            renderingData.renderLayerMask = UInt32.MaxValue;
            ...
            if (cameraData.isUiCamera)
            {
                ...
                renderingData.perObjectData = new PerObjectData();
            }
            else
            {
                ...
                if (cameraData.maxShadowDistance > 0.0f)
                {
                    ...
                    if (settings.additionalLightsRenderingMode == LightRenderingMode.PerPixel)
                    {
                        for (int i = 0; i < visibleLights.Length; ++i)
                        {
                            ...
                            if (visibleLights[i].lightType == LightType.Spot && light != null &&
                ...
                InitializeShadowData(settings, visibleLights, mainLightCastShadows,
            ...
            bool mainLightCastShadows, bool additionalLightsCastShadows, out ShadowData shadowData)
        {
            ...
            shadowData.supportsMainLightShadows = SystemInfo.supportsShadows && settings.supportsMainLightShadows &&
            ...
            switch (shadowData.mainLightShadowCascadesCount)
            {
                ...
                case 3:
                    shadowData.mainLightShadowCascadesSplit =
                        new Vector3(settings.cascade3Split.x, settings.cascade3Split.y, 0.0f);
                    break;
            ...
            shadowData.supportsAdditionalLightShadows = SystemInfo.supportsShadows &&
        ...
        static void InitializePostProcessingData(UniversalRenderPipelineAsset settings,
        ...
        static void InitializeLightData(UniversalRenderPipelineAsset settings, NativeArray<VisibleLight> visibleLights,
            int mainLightIndex, out LightData lightData)
        {
            ...
            if (settings.additionalLightsRenderingMode != LightRenderingMode.Disabled)
            {
                ...
                lightData.maxPerObjectAdditionalLightsCount =
                    Math.Min(settings.maxAdditionalLightsCount, maxPerObjectAdditionalLights);
            }
            else
            {
            ...
            lightData.renderLayerMask = UInt32.MaxValue;
        }
        ...
        static void SetupPerFrameShaderConstants()
        {
            ...
            MiscConfig oc = WorldSystem.miscConfig;
            if (oc)
            {
                Vector4 dist = new Vector4(
                    oc.defaultDist.minDist,
                    oc.defaultDist.maxDist,
                    oc.defaultDist.minScale,
                    oc.defaultDist.maxScale);
                Shader.SetGlobalVector(ShaderPropertyId.outlineScale, dist);
                Shader.SetGlobalVector(ShaderPropertyId.roleRamp, new Vector4(oc.lightFaceRampOffset, oc.darkFaceRampOffset, 0, 0));
                Shader.SetGlobalVector(ShaderPropertyId.rimParam, oc.rimParam);
                Shader.SetGlobalVector(ShaderPropertyId.lightFadePram, oc.lightFadePram);
            }
            
            GlobalShaderEffects.Update();
        }
 ...
 SortingCriteria.SortingLayer | SortingCriteria.RenderQueue | SortingCriteria.OptimizeStateChanges | SortingCriteria.CanvasOrder;
            ...
            var MaxShadowDistanceMultiplier =
 AdaptivePerformance.AdaptivePerformanceRenderSettings.MaxShadowDistanceMultiplier;
 ...
 // (int)cameraData.antialiasingQuality - AdaptivePerformance.AdaptivePerformanceRenderSettings.AntiAliasingQualityBias;
 //            if (antialiasingQualityIndex < 0)
 //                cameraData.antialiasing = AntialiasingMode.None;
 //            cameraData.antialiasingQuality =
 // (AntialiasingQuality)Mathf.Clamp(antialiasingQualityIndex, (int)AntialiasingQuality.Low, (int)AntialiasingQuality.High);
        }
        static void ApplyAdaptivePerformance(ref RenderingData renderingData)
        {
            ...
            var MainLightShadowmapResolutionMultiplier =
 AdaptivePerformance.AdaptivePerformanceRenderSettings.MainLightShadowmapResolutionMultiplier;
            renderingData.shadowData.mainLightShadowmapWidth =
 (int)(renderingData.shadowData.mainLightShadowmapWidth * MainLightShadowmapResolutionMultiplier);
            renderingData.shadowData.mainLightShadowmapHeight =
 (int)(renderingData.shadowData.mainLightShadowmapHeight * MainLightShadowmapResolutionMultiplier);

            var MainLightShadowCascadesCountBias =
 AdaptivePerformance.AdaptivePerformanceRenderSettings.MainLightShadowCascadesCountBias;
            renderingData.shadowData.mainLightShadowCascadesCount =
 Mathf.Clamp(renderingData.shadowData.mainLightShadowCascadesCount - MainLightShadowCascadesCountBias, 0, 4);

Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        //#pragma exclude_renderers gles
        #pragma multi_compile _ _CUSTOM_DISTORTION
        //#pragma multi_compile_local_fragment _ _DISTORTION
        //#pragma multi_compile_local_fragment _ _CHROMATIC_ABERRATION
        #pragma multi_compile_local_fragment _ _BLOOM
        #pragma multi_compile_local_fragment _ _GODRAY
        //#pragma multi_compile_local_fragment _ _HDR_GRADING _TONEMAP_ACES _TONEMAP_CUSTOM _TONEMAP_NEUTRAL
        #pragma multi_compile_local_fragment _ _TONEMAP_CUSTOM
        //#pragma multi_compile_local_fragment _ _FILM_GRAIN
        //#pragma multi_compile_local_fragment _ _DITHERING
        #pragma multi_compile_local_fragment _ _LINEAR_TO_SRGB_CONVERSION
    
        #pragma multi_compile_local_fragment _ _RADIAL_BLUR
        #pragma multi_compile_local_fragment _ _FXAA

        //#pragma multi_compile_local_fragment _ _USE_DRAW_PROCEDURAL
        //#pragma multi_compile_local_fragment _ _3DUIBACKGROUND

        #include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Common.hlsl"
        #include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Filtering.hlsl"
        #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
        #include "Packages/com.unity.render-pipelines.universal/Shaders/PostProcessing/Common.hlsl"

        // Hardcoded dependencies to reduce the number of variants
        /*
        #if _BLOOM_LQ || _BLOOM_HQ || _BLOOM_LQ_DIRT || _BLOOM_HQ_DIRT
            #define BLOOM
            #if _BLOOM_LQ_DIRT || _BLOOM_HQ_DIRT
                #define BLOOM_DIRT
            #endif
        #endif
        */
    
        //TEXTURE2D_X(_3DUIBackGround);
        TEXTURE2D_X(_SourceTex);
        TEXTURE2D_X(_Bloom_Texture);
        TEXTURE2D_X(_GodRayTex);
        TEXTURE2D(_LensDirt_Texture);
        TEXTURE2D(_Grain_Texture);
        TEXTURE2D(_InternalLut);
        TEXTURE2D(_UserLut);
        TEXTURE2D(_BlueNoise_Texture);

    #if defined(_CUSTOM_DISTORTION)
        TEXTURE2D(_URPDistortionTex);
    #endif
        TEXTURE2D(_RadialBlurTex);
        float _Fxaa_Sharp;
        float4 _Lut_Params;
        float4 _UserLut_Params;
        float4 _Bloom_Params;
        float _Bloom_RGBM;
        half _GodRayIntensity;
        float4 _LensDirt_Params;
        float _LensDirt_Intensity;
        float4 _Distortion_Params1;
        float4 _Distortion_Params2;
        float _Chroma_Params;
        half4 _Vignette_Params1;
        float4 _Vignette_Params2;
        float2 _Grain_Params;
        float4 _Grain_TilingParams;
        float4 _Bloom_Texture_TexelSize;
        float4 _SourceTex_TexelSize;
        float4 _Dithering_Params;

    //Tmp=====================
        float _BloomLimit;
        float _BloomOffsetStrength;
        half tonemapping_enable;
    //========================

        #define DistCenter              _Distortion_Params1.xy
        #define DistAxis                _Distortion_Params1.zw
        #define DistTheta               _Distortion_Params2.x
        #define DistSigma               _Distortion_Params2.y
        #define DistScale               _Distortion_Params2.z
        #define DistIntensity           _Distortion_Params2.w

        #define ChromaAmount            _Chroma_Params.x

        #define BloomIntensity          _Bloom_Params.x
        #define BloomTint               _Bloom_Params.yzw
        #define BloomRGBM               _Bloom_RGBM.x
        #define LensDirtScale           _LensDirt_Params.xy
        #define LensDirtOffset          _LensDirt_Params.zw
        #define LensDirtIntensity       _LensDirt_Intensity.x

        #if defined(USE_URP_VIGNETTE)
            #define VignetteColor           _Vignette_Params1.xyz
            #define VignetteCenter          _Vignette_Params2.xy
            #define VignetteIntensity       _Vignette_Params2.z
            #define VignetteSmoothness      _Vignette_Params2.w
            #define VignetteRoundness       _Vignette_Params1.w
        #else
            // Vignette
            half4 _Vignette_Settings;
            #define vignette_enable _Vignette_Settings.x>0
            #define vignette_intensity _Vignette_Settings.x
            #define vignette_smoothness _Vignette_Settings.y
            #define vignette_roundness _Vignette_Settings.z
            #define vignette_rounded _Vignette_Settings.w
		    half3 _Vignette_Color;
		    half2 _Vignette_Center; // UV space
        #endif

        #define LutParams               _Lut_Params.xyz
        #define PostExposure            _Lut_Params.w
        #define UserLutParams           _UserLut_Params.xyz
        #define UserLutContribution     _UserLut_Params.w

        #define GrainIntensity          _Grain_Params.x
        #define GrainResponse           _Grain_Params.y
        #define GrainScale              _Grain_TilingParams.xy
        #define GrainOffset             _Grain_TilingParams.zw

        #define DitheringScale          _Dithering_Params.xy
        #define DitheringOffset         _Dithering_Params.zw

    #if defined(_CUSTOM_DISTORTION)
        float2 CustomDistortionUV(float2 uv)
        {
            float3 distortion = SAMPLE_TEXTURE2D(_URPDistortionTex, sampler_LinearClamp, uv).xyz;
			uv += (distortion.xy - 0.5) * distortion.z;
            return uv;
        }
    #endif
        float2 DistortUV(float2 uv)
        {
            // Note: this variant should never be set with XR
            #if _DISTORTION
            {
                uv = (uv - 0.5) * DistScale + 0.5;
                float2 ruv = DistAxis * (uv - 0.5 - DistCenter);
                float ru = length(float2(ruv));

                UNITY_BRANCH
                if (DistIntensity > 0.0)
                {
                    float wu = ru * DistTheta;
                    ru = tan(wu) * (rcp(ru * DistSigma));
                    uv = uv + ruv * (ru - 1.0);
                }
                else
                {
                    ru = rcp(ru) * DistTheta * atan(ru * DistSigma);
                    uv = uv + ruv * (ru - 1.0);
                }
            }
            #endif

            return uv;
        }

        half4 Frag(Varyings input) : SV_Target
        {
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);

            float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
        #if defined(_CUSTOM_DISTORTION)
            uv = CustomDistortionUV(uv);
        #endif
            float2 uvDistorted = DistortUV(uv);

            half3 color = (0.0).xxx;
            half4 colorBackGround = (0.0).xxxx;
            half alpha = 0.0;

            half4 color4 = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uvDistorted);
            color = color4.xyz;
            alpha = color4.w;

#if _3DUIBACKGROUND
            colorBackGround = SAMPLE_TEXTURE2D_X(_3DUIBackGround, sampler_LinearClamp, uvDistorted);
#endif

#if _3DUIBACKGROUND
            color = lerp(colorBackGround.xyz, color, alpha);
            alpha = max(colorBackGround.w,alpha);
#endif

            #if defined(_RADIAL_BLUR)
                half4 radialBlur = SAMPLE_TEXTURE2D_X(_RadialBlurTex, sampler_LinearClamp, uvDistorted);
                color.rgb = lerp(color.rgb, radialBlur.rgb, radialBlur.a);
             #endif

            // Gamma space... Just do the rest of Uber in linear and convert back to sRGB at the end
            #if UNITY_COLORSPACE_GAMMA
            {
                color = SRGBToLinear(color);
            }
            #endif

            #if defined(_GODRAY)
            {
                half4 godray = SAMPLE_TEXTURE2D_X(_GodRayTex, sampler_LinearClamp, uvDistorted);
                color += godray * _GodRayIntensity;
            }
            #endif

            #if defined(_BLOOM)
            {
                float4 bloom = SAMPLE_TEXTURE2D_X(_Bloom_Texture, sampler_LinearClamp, uvDistorted);
                float luminance = saturate(Luminance(bloom) * 0.5);//经特效核对确认锁定在0.27
                luminance = lerp(luminance, saturate(luminance), _BloomLimit);
                color = max(0, color * (1-luminance) + bloom * (1+luminance));
                // color = color + bloom;
            }
            #endif


            #if defined(USE_URP_VIGNETTE)
            // To save on variants we'll use an uniform branch for vignette. Lower end platforms
            // don't like these but if we're running Uber it means we're running more expensive
            // effects anyway. Lower-end devices would limit themselves to on-tile compatible effect
            // and thus this shouldn't too much of a problem (famous last words).
            UNITY_BRANCH
            if (VignetteIntensity > 0)
            {
                color = ApplyVignette(color, uvDistorted, VignetteCenter, VignetteIntensity, VignetteRoundness, VignetteSmoothness, VignetteColor);
            }
            #else
            UNITY_BRANCH
            if (vignette_enable)
            {
                color = ApplyVignetteLegacy(color, uvDistorted, _Vignette_Center, vignette_intensity, vignette_roundness, vignette_smoothness, vignette_rounded, _Vignette_Color);                
            }
            #endif

            UNITY_BRANCH
            if (tonemapping_enable > 0)
            {
                color = ApplyColorGrading(color, PostExposure, TEXTURE2D_ARGS(_InternalLut, sampler_LinearClamp), LutParams, TEXTURE2D_ARGS(_UserLut, sampler_LinearClamp), UserLutParams, UserLutContribution);
            }

            #if _FILM_GRAIN
            {
                color = ApplyGrain(color, uv, TEXTURE2D_ARGS(_Grain_Texture, sampler_LinearRepeat), GrainIntensity, GrainResponse, GrainScale, GrainOffset);
            }
            #endif

            // Back to sRGB
            #if UNITY_COLORSPACE_GAMMA || _LINEAR_TO_SRGB_CONVERSION
            {
                color = LinearToSRGB(color);
            }
            #endif

            #if _DITHERING
            {
                color = ApplyDithering(color, uv, TEXTURE2D_ARGS(_BlueNoise_Texture, sampler_PointRepeat), DitheringScale, DitheringOffset);
            }
            #endif

            #if defined(_FXAA)
                alpha = dot(color.rgb, float3(0.333, 0.6544, 0));
            #endif
            return half4(color, alpha);
        }

    ENDHLSL

    SubShader
    {
        Tags { "RenderType" = "Opaque" "RenderPipeline" = "UniversalPipeline"}
        LOD 100
        ZTest Always ZWrite Off Cull Off

        Pass
        {
            Name "UberPost"

            HLSLPROGRAM
                #pragma vertex FullscreenVert
                #pragma fragment Frag
            ENDHLSL
        }
    }
}

...
half3 ApplyVignetteLegacy(half3 input, float2 uv, float2 center, float intensity, float roundness, float smoothness, half rounded, half3 color)
{
    half2 d = abs(uv - center) * intensity;
    d.x *= lerp(1.0, _ScreenParams.x / _ScreenParams.y, rounded);
    d = pow(saturate(d), roundness); // Roundness
    FLOAT vfactor = pow(saturate(1.0 - dot(d, d)), smoothness);
    input = saturate(input);
    input.rgb *= lerp(color, (1.0).xxx, vfactor);
    // Alpha现在没有用到。
    //input.a = lerp(1.0, input.a, vfactor);
    return input.rgb; 
}

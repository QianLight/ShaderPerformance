...
namespace UnityEditor.Rendering.Universal
{
    ...
    class UniversalRenderPipelineCameraEditor : CameraEditor
    {
        ...
        static class Styles
        {
            ...
            //Custom
            public static GUIContent custom = EditorGUIUtility.TrTextContent("Custom", "");
            public static GUIContent waterLight = EditorGUIUtility.TrTextContent("Water Light Dir (Transform)", "A transform rotation that will act as a water light dir");
...
#endif
        SerializedProperty m_AdditionalCameraDataWaterLightDir;
        ...
        public new void OnEnable()
        {
            ...
            m_CustomFoldout = new SavedBool($"{target.GetType()}.CustomFoldout", false);
...
#endif
            m_AdditionalCameraDataWaterLightDir = m_AdditionalCameraDataSO.FindProperty("m_WaterLightDir");
        }
        ...
        void DrawCustom()
        {
            m_CustomFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_CustomFoldout.value, Styles.custom);
        ...
        void DrawWaterLight()
        {
            ...
            Transform selectedWaterLightDir;
            if (m_AdditionalCameraDataSO == null)
            {
                selectedWaterLightDir = null;
            }
            else
            {
                selectedWaterLightDir = (Transform)m_AdditionalCameraDataWaterLightDir.objectReferenceValue;
            }

            hasChanged |= DrawObjectField(m_AdditionalCameraDataWaterLightDir, ref selectedWaterLightDir, Styles.waterLight);

            if (hasChanged)
            {
                m_AdditionalCameraDataWaterLightDir.objectReferenceValue = selectedWaterLightDir;
                m_AdditionalCameraDataSO.ApplyModifiedProperties();
            }
        }
        ...
        void DrawTargetTexture(UniversalRenderPipelineAsset rpAsset)
        {
            ...
            if (!settings.targetTexture.hasMultipleDifferentValues && rpAsset != null)
            {
                ...
                int pipelineSamplesCount = UniversalRenderPipeline.msaaSampleCount;
        ...
        void DrawPostProcessing()
        {
            ...
            EditorGUILayout.HelpBox("抗锯齿选项在URPAsset中调整", MessageType.Info);
            if (GUILayout.Button("跳转", GUILayout.Width(40)))
            {
                const string urpAssetPath = "Assets/Engine/Runtime/Shaders/Shading/URPAsset.asset";
                Selection.activeObject = AssetDatabase.LoadAssetAtPath<UniversalRenderPipelineAsset>(urpAssetPath);
            }
            ...
            //bool msaaEnabled = UniversalRenderPipeline.msaaSampleCount > 1;
            //AntialiasingMode fallbackAA = (AntialiasingMode)UniversalRenderPipeline.asset.fallbackAA; 
            //AntialiasingMode msaaMode = msaaEnabled ? fallbackAA : AntialiasingMode.FastApproximateAntialiasing;
            //m_AdditionalCameraDataAntialiasing.intValue = (int)msaaMode;

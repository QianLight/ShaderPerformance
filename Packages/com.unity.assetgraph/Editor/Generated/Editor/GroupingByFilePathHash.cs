...
{
	[CustomNode("Group Assets/Group By File Path Hash", 42)]
	public class GroupingByFilePathHash : Node {
		...
		public override Node Clone(Model.NodeData newData) {
            var newNode = new GroupingByFilePathHash();
                        ...
                        foreach(var a in assets) {
                            ...
                            if (!string.IsNullOrEmpty (m_groupNameFormat [target])) {
                                key = m_groupNameFormat [target]
                                    ...
                                    .Replace ("{OldGroup}", g)
                                    .Replace ("{FilePathHash}", StringUtility.GetHashCode(a.path).ToString("X4").ToLower());

                            }
                            if(outputDict.ContainsKey(key)){
                                throw new Exception("GroupingByFilePathHash has duplicate group name! " + key);
                            }

    ...
    public class AssetGraphEditorWindow : EditorWindow
    {
		...
		*/
        ...
        public static void DeleteCache ()
        {
            ...
            FileUtility.RemakeDirectory(Model.Settings.Path.BundleBuilderCachePath);
        ...
        private void Run ()
        {
            ...
            Node.IsBuild = true;
            System.DateTime preTime = System.DateTime.Now;
            try {
                ...
                EditorUtility.ClearProgressBar ();
                Node.IsBuild = false;
                Debug.Log("Build Bundle cost " + (System.DateTime.Now - preTime).TotalSeconds + "s");
            }
        }
        ...
        private void DrawGUIToolBar ()
        {
            using (new EditorGUILayout.HorizontalScope (EditorStyles.toolbar)) {
                ...
                if (GUILayout.Button("Save", EditorStyles.toolbarButton, GUILayout.Height(Model.Settings.GUI.TOOLBAR_HEIGHT)))
                {
                    AssetDatabase.SaveAssets();
                }
		...
		*/
        ...
        [MenuItem("Zeus/AssetGraph/BuildAssetBundle", false, 2)]
        public static void BuildAssetBundleForCurrentPlatform()
        {
            int  choice = EditorUtility.DisplayDialogComplex ("生成AssetBundle", "是否要重新生成全部?", "是", "取消", "否");
            switch (choice)
            {
                case 0: 
                    BuileAssetBundle(EditorUserBuildSettings.activeBuildTarget, true);
                    break;
                case 1:
                    return;
                case 2:
                    BuileAssetBundle(EditorUserBuildSettings.activeBuildTarget, false);
                    break;
            }
        }

        public static void BuileAssetBundle(BuildTarget target, bool isRebuildAll)
        {
            if (isRebuildAll)
            {
                UnityEngine.AssetGraph.AssetGraphEditorWindow.DeleteCache();
            }
            UnityEngine.AssetGraph.ExecuteGraphResult result = UnityEngine.AssetGraph.AssetGraphUtility.ExecuteGraph(target, "Assets/AssetGraph/AssetGraph.asset");
            if (result.IsAnyIssueFound)
            {
                foreach (var issue in result.Issues)
                {
                    Debug.LogError("Build AssetGraph Exception, reason: " + issue.Reason + " fix " + issue.HowToFix);
                    Debug.LogException(issue);
                }
                throw new Exception("AssetGraph build failed.");
            }
        }

...
namespace UnityEngine.AssetGraph {
    ...
    public class ExtractSharedAssets : Node {
    		...
    		PerformGraph.Output Output) 
    	{
    		...
    		if(Output != null) {
    			...
    			if(incoming != null) {
                    ...
                    var hadSpecifyGroup = new HashSet<string>();
                    ...
                    foreach (var ag in incoming)
                    {
                        foreach (var key in ag.assetGroups.Keys)
                        {
                            ...
                            {
                                hadSpecifyGroup.Add(a.importFrom);
                            }
                        }
                    }
    				...
    				foreach(var ag in incoming) {
    					foreach (var key in ag.assetGroups.Keys) {
    						...
    						foreach(var a in assets) {
    							CollectDependencies(key, a, ref dependencyCollector, hadSpecifyGroup);
    						}
    					}
    				}
    	...
    	private void CollectDependencies(string groupKey, AssetReference asset, ref Dictionary<string, List<string>> collector, HashSet<string> removes = null) {

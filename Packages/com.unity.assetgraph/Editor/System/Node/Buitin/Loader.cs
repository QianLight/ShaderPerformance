	...
	public class Loader : Node, Model.NodeDataImporter {
        ...
        public static string NormalizeLoadPath(string path)
        {
            if (!string.IsNullOrEmpty(path))
            {
                ...
                string packagePath = Path.Combine(Path.GetDirectoryName(dataPath), "Packages").Replace("\\", "/");
                if (dataPath == path)
                {
                    path = "Assets";
                }
                else if(packagePath == path)
                {
                    path = "Packages";
                }
                else if(path.Contains(dataPath) || path.Contains(packagePath))
                {
                    path = path.Substring(Path.GetDirectoryName(dataPath).Length + 1);
                }
            }
            return path;
        }
        ...
        public override void OnInspectorGUI(NodeGUI node, AssetReferenceStreamManager streamManager, NodeGUIEditor editor, Action onValueChanged) {
			...
			using (new EditorGUILayout.VerticalScope(GUI.skin.box)) {
				...
				using (disabledScope) {
                    ...
                    newLoadPath = editor.DrawFolderSelector ("", "Select Asset Folder", 
                        path,
                        path,
                        (string folderSelected) => { return NormalizeLoadPath(folderSelected); }
                    );

                    var dirPath = newLoadPath;
					...
					if(!dirExists && !string.IsNullOrEmpty(dirPath)) {
			...
			PerformGraph.Output Output) 
		{
            ...
            ValidateLoadPath(
                ...
                () => {
                    throw new NodeException(
                        "Directory path contains '\\': " + GetLoaderFullLoadPath(target),
			...
			PerformGraph.Output Output) 
		{
            ...
            {
                int index = loadPath.IndexOf("Packages");
                if(index > -1)
                {
                    targetFiles = AssetDatabase.FindAssets("", new string[] { loadPath.Substring(index) });
                }
                else
                {
                    Debug.LogError("If you want to choose a package path out of project, please make sure it is under the \"Packages\" folder.");
                    return;
                }
            }
            else
            {
                targetFiles = AssetDatabase.FindAssets("",  new string[] { loadPath });
            }
            HashSet<string> newTargetFiles = new HashSet<string>();

            foreach (var assetGuid in targetFiles) {
                if (newTargetFiles.Contains(assetGuid))
                {
                    continue;
                }
                newTargetFiles.Add(assetGuid);
                ...
                {
                    throw new Exception("targetFilePath is not a file and a directory,targetFilePath =" + targetFilePath);
                }
		...
		public static void ValidateLoadPath (string currentLoadPath, string combinedPath, Action NullOrEmpty, Action NotExist, Action Invalid) {
			if (string.IsNullOrEmpty(currentLoadPath)) NullOrEmpty();
#if UNITY_EDITOR_OSX
            if (currentLoadPath.Contains("\\")) Invalid();
#endif
		...
		private string GetLoaderFullLoadPath(BuildTarget g) {
            if(string.IsNullOrEmpty(m_loadPath[g]))
            {
                return Directory.GetCurrentDirectory();
            }
			return Path.GetFullPath(m_loadPath[g]);
		}

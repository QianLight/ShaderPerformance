...
namespace UnityEngine.AssetGraph
{
    ...
    public class PlayerBuilder : Node
    {
        ...
        [SerializeField] private SerializableMultiTargetInt m_subTarget;
        ...
        public override void Initialize(Model.NodeData data)
        {
            ...
            m_subTarget = new SerializableMultiTargetInt();
        ...
        public override Node Clone(Model.NodeData newData)
        {
            ...
            newNode.m_subTarget = new SerializableMultiTargetInt(m_subTarget);
            ...
            using (new EditorGUILayout.VerticalScope(GUI.skin.box))
            {
                var disabledScope = editor.DrawOverrideTargetToggle(node, m_buildOptions.ContainsValueOf(editor.CurrentEditingGroup), (bool enabled) =>
                {
                    using (new RecordUndoScope("Remove Target Build Settings", node, true))
                    {
                        if (enabled)
                        {
                            ...
                            m_subTarget[editor.CurrentEditingGroup] = m_subTarget.DefaultValue;
                        }
                        else
                        {
                            ...
                            m_subTarget.Remove(editor.CurrentEditingGroup);
                        }
                        onValueChanged();
                    }
                });
                ...
                using (disabledScope)
                {
                    using (var scrollScope = new EditorGUILayout.ScrollViewScope(m_scroll))
                    {
                        ...
                        GUILayout.Space(4f);
                        var subTarget = (StandaloneBuildSubtarget)EditorGUILayout.EnumPopup("subTarget", (StandaloneBuildSubtarget)m_subTarget[editor.CurrentEditingGroup]);
                        if ((int)subTarget != m_subTarget[editor.CurrentEditingGroup])
                        {
                            using (new RecordUndoScope("Change SubTarget", node, true))
                            {
                                m_subTarget[editor.CurrentEditingGroup] = (int)subTarget;
                                onValueChanged();
                            }
                        }
#endif
                        ...
                        if (scenesInProject.Length > 0)
                        {
                            ...
                            using (new EditorGUILayout.VerticalScope(GUI.skin.box))
                            {
                                ...
                                foreach (var sceneGUID in scenesInProject)
                                {
                                    ...
                                    var type = TypeUtility.GetMainAssetFuzzyTypeAtPath(path);

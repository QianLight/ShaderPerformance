...
{
    using BindType = BlueprintEmmyTLM.BindType;

    public class ToLuaBlueprintAPIGenerator
    {
        [MenuItem("Blueprint/Gen API for Blueprint")]
        static public void DoIt()
        {
            Filter<Type> baseFilter = new GeneralFilter<Type>(BlueprintEmmyTLM.baseType);
            Filter<Type> dropFilter = new GeneralFilter<Type>(BlueprintEmmyTLM.dropType);
            var directory = Directory.CreateDirectory("LuaAPI");
            if (BlueprintEmmyCS.customTypeList == null)
                return;
            var collection = new BindTypeCollection(BlueprintEmmyCS.customTypeList);
    ...
    class BlackListMemberNameFilter<T> : Filter<T> where T : MemberInfo
    {
        ...
        {
            if (BlueprintEmmyTLE.memberFilter.Contains(mi.Name))
                return true;
            var type = mi.ReflectedType;
            if (type != null)
                return BlueprintEmmyTLE.memberFilter.Contains(type.Name + "." + mi.Name);
            return false;
        }
    }
    ...
    abstract class CodeGenerator
    {
        ...
        protected abstract void GenConstructor(string name, ConstructorInfo[] ctors);
    }
    ...
    static class TypeExtension
    {
        public static string GetTypeStr(this Type type)
        {
            ...
            {
                var typeName = BlueprintEmmyLM.GetTypeName(type);
                int pos = typeName.IndexOf("<", StringComparison.Ordinal);
                if (pos > 0)
                    return typeName.Substring(0, pos);
            }
            return BlueprintEmmyLM.GetTypeName(type);
        }
    }
    ...
    class LuaAPIGenerator : CodeGenerator
    {
        ...
        protected override void GenProperty(PropertyInfo pi)
        {
            if ((pi.GetMethod != null) && ((pi.GetMethod.Attributes & MethodAttributes.Static) != 0))
                _propBuilder.AppendFormat("---@field {0} {1} static{2}\n", pi.Name, pi.PropertyType.GetTypeStr(), (pi.CanWrite ? string.Empty : " readonly"));
            else
                _propBuilder.AppendFormat("---@field {0} {1}{2}\n", pi.Name, pi.PropertyType.GetTypeStr(), (pi.CanWrite ? string.Empty : " readonly"));
        }
        ...
        protected override void GenField(FieldInfo fi)
        {
            if ((fi.Attributes & FieldAttributes.Static) != 0)
                _propBuilder.AppendFormat("---@field {0} {1} static\n", fi.Name, fi.FieldType.GetTypeStr());
            else
                _propBuilder.AppendFormat("---@field {0} {1}\n", fi.Name, fi.FieldType.GetTypeStr());
        }

        protected override void GenConstructor(string name, ConstructorInfo[] ctors)
        {

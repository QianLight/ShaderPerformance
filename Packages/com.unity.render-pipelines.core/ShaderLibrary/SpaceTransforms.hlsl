...
// Tranforms position from homogenous space to world space
// Add by :     Takeshi
// Date:        2022/08/25
// Note :       Transforming positionCS to positionWS with UNITY_MATRIX_I_VP
//              will lose data precision and result in some BUGs.
//              The safe way to transformation is to transform positionCS to positionVS first,
//              Then to transform positionVS to positionWS.             
float3 TransformHClipToWorld(float4 positionCS)
{
    float3 positionVS = mul(UNITY_MATRIX_I_P, positionCS).xyz;

...
namespace UnityEditor.Rendering
{
    ...
    {
        public static VolumeProfile currentProfile { get; private set; } 
        public static Volume currentVolume { get; private set; }

        SerializedProperty m_IsGlobal;
        SerializedProperty m_IsTimeline;
        ...
        readonly GUIContent[] m_Modes = { new GUIContent("Global"), new GUIContent("Local"), new GUIContent("Timeline")};
        ...
        void OnEnable()
        {
            ...
            try
            {
                ...
                m_IsTimeline = o.Find(x => x.isTimeline);
            ...
            {
                Debug.LogWarning(e); // serializedObject 在编辑器里有一些异常，暂时没有找到原因。
            }
#endif
        }
        ...
        {
            currentVolume = target as Volume;
            currentProfile = (currentVolume).sharedProfile;
                
            serializedObject.Update();

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Gizmo style");
            Volume.VolumeGizmosStyle m_GizmosStyle = Volume.GetVolumeGizmosStyle();
            Volume.VolumeGizmosStyle lastVolumeGizmosStyle = m_GizmosStyle;
            m_GizmosStyle = (Volume.VolumeGizmosStyle)EditorGUILayout.EnumPopup(m_GizmosStyle);
            if (lastVolumeGizmosStyle != m_GizmosStyle)
            {
                Volume.SetVolumeGizmosStyle(m_GizmosStyle);
            }
            EditorGUILayout.EndHorizontal();

            GUIContent label = EditorGUIUtility.TrTextContent("Mode", "A global volume is applied to the whole scene.");
            Rect lineRect = EditorGUILayout.GetControlRect();
            // int isTimeline = m_IsTimeline.boolValue ? 0 : 1;
            ...
            EditorGUI.BeginProperty(lineRect, label, m_IsGlobal);
            {
                ...
                mode = EditorGUI.Popup(lineRect, label, mode, m_Modes);
                if (EditorGUI.EndChangeCheck())
                {
                    m_IsGlobal.boolValue = mode != 1;
                    m_IsTimeline.boolValue = mode == 2;
                }
            }
            EditorGUI.EndProperty();

            if (mode == 1) // Blend radius is not needed for global volumes
            {
            ...
            VolumeComponentEditor.developMode.Value = EditorGUILayout.Toggle("Develop Mode", VolumeComponentEditor.developMode.Value);

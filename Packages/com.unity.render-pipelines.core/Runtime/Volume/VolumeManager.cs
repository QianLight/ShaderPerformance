...
namespace UnityEngine.Rendering
{
    ...
    public sealed class VolumeManager
    {
        ...
        public Action<VolumeStack> onOverrideFinish;

        /// <summary>
        /// 是否强制更新 Volume
        /// </summary>
        public bool isForceUpdateVolume;

#if UNITY_EDITOR
        private bool isInSkillEditor;
#endif
        VolumeManager()
        {
        ...
        public void Register(Volume volume, int layer)
        {
            if (volume.profileRef != null)
            {
                foreach (var component in volume.profileRef.components)
                {
                    ...
                    component.Target = volume.gameObject.transform;
                }
            }
        ...
        public Volume GetVolume(string volName)
        {
            Volume targetVol = null;
            foreach(Volume vol in m_Volumes)
            {
                if(vol.gameObject.name == volName)
                {
                    targetVol = vol;
                }
            }
        ...
        void OnFireChange(Volume volume, Camera camera, Transform root, List<VolumeComponent> components, bool fire)
        {
            if (components != null)
            {
                foreach (var component in components)
                {
                    ...
                    component.OnFireChange(camera, root, fire);
                }
            }
            changeVolume = true;
            changeVolumeForVolumeData = true;
            // foreach (var component in stack.components.Values)
            // {
            //     component.OnOverrideFinish();
            // }
            //Debug.LogWarning("VolumeManager.OnFireChange:" + volume + "," + volume.priority + "," + volume.isGlobal + "," + volume.isTimeline + "," + fire);
#if UNITY_EDITOR
            isInSkillEditor = SceneManagement.SceneManager.GetActiveScene().name == "Role_screencapture";
#endif
        }

        void CheckDistanceVolume(Camera camera, Volume volume, bool isMainCamera)
        {
            if (camera != null && isMainCamera)
            {
                //Volume change
                if (lastVolume != volume)
                {
                    if (lastVolume != null)
                    {
                        GameObject obj = lastVolume.gameObject;
                        VolumeProfile vp = lastVolume.profileRef;
                        OnFireChange(lastVolume, camera, obj == null ? null : obj.transform, vp == null ? null : vp.components, false);
                    }

                    if (volume != null)
                    {
                        callbackVolume = volume;
                    }
                    lastVolume = volume;
                }
                if (callbackVolume != null)
                {
                    GameObject obj = callbackVolume.gameObject;
                    VolumeProfile vp = callbackVolume.profileRef;
                    OnFireChange(callbackVolume, camera, obj == null ? null : obj.transform, vp == null ? null : vp.components, true);
                    if (callbackVolume.gameObject != null && callbackVolume.gameObject.scene != null && callbackVolume.gameObject.scene.isLoaded)
                        SceneManagement.SceneManager.SetActiveScene(callbackVolume.gameObject.scene);
                    callbackVolume = null;
                }
            }
        }
        ...
        // Go through all listed components and lerp overridden values in the global state
        void OverrideData(VolumeStack stack, Volume volume, float interpFactor, bool update)
        {
            if (!update)
                return;
            foreach (var component in volume.profileRef.components)
        ...
        private VolumeComponent _ReplaceDataItm;
        private VolumeComponent _targetItm;
        private VolumeParameter _VolumeParameterItm;

        // Faster version of OverrideData to force replace values in the global state
        void ReplaceData(VolumeStack stack, List<VolumeComponent> components, bool update)
        {
            if (!update)
                return;
            for (int j = 0; j < components.Count; j++)
            {
                _ReplaceDataItm = components[j];
                _targetItm = stack.GetComponent(_ReplaceDataItm.GetType());
                int count = _ReplaceDataItm.parameters.Count;

                for (int i = 0; i < count; i++)
                {
                    _VolumeParameterItm = _targetItm.parameters[i];

                    if (_VolumeParameterItm != null)
                    {
                        _VolumeParameterItm.overrideState = false;
                        _VolumeParameterItm.SetValue(_ReplaceDataItm.parameters[i]);
                    }
                }
            }
        }
        ...
        {
#if UNITY_EDITOR
            if (UnityEditor.EditorApplication.isPlaying && !firstCheck)
            {
                firstCheck = true;
            }
...
#endif
        public delegate void StrategicUpadte(Camera camera);
        public StrategicUpadte VolumeStrategicUpadte = camera => { };
        ...
        /// for blending.</param>
        public void Update(Transform trigger, LayerMask layerMask, bool debug, bool mainCamera)
        {
            Update(stack, trigger, layerMask, debug, mainCamera);
        }

        public bool ChangeColorGradingLUT(bool setState = false, bool state = false)
        {
            bool lastValue = changeVolumeForColorGradingLUT;
            if (setState)
            {
                changeVolumeForColorGradingLUT = state;
            }
            return lastValue;
        }
        private bool changeVolume = true, changeVolumeForColorGradingLUT = true, changeVolumeForVolumeData = true;
        ...
        /// for blending.</param>
        /// <seealso cref="VolumeStack"/>
        public void Update(VolumeStack stack, Transform trigger, LayerMask layerMask, bool debug, bool mainCamera)
        {
            bool needUpdate = changeVolumeForVolumeData;
            needUpdate |= isForceUpdateVolume;
#if UNITY_EDITOR
            if (mainCamera)
            {
                if(debug)
                {
                    if (needUpdate)
                    {
                        changeVolumeForVolumeData = false;
                    }
                    if (changeVolume)
                    {
                        changeVolume = false;
                        changeVolumeForColorGradingLUT = true;
                    }
                }
                else
                {
            ...
            ReplaceData(stack, m_ComponentsDefaultState, needUpdate);
            ...
            float floatMax = 200000000;
            float targetDistance = floatMax;
            Volume minDistanceVolume = null;
            ...
            foreach (var volume in volumes)
            {
                ...
                if (volume.isGlobal)
                {
                    OverrideData(stack, volume, Mathf.Clamp01(volume.weight), needUpdate);

                    if(volume.isTimeline)
                    {
                        changeVolumeForVolumeData = true;
                    }
                    else
                    {
                        ...
                        {
                            if (globalVolume.priority < volume.priority)
                            {
                                globalVolume = volume;
                            }
                        }
                ...
                float closestDistanceMin = float.MaxValue;
                foreach (var collider in colliders)
                {
                    ...
                    if (d < closestDistanceMin)
                        closestDistanceMin = d;
                }
                ...
                OverrideData(stack, volume, interpFactor * Mathf.Clamp01(volume.weight), needUpdate);

                if (closestDistanceMin <= targetDistance)
                {
                    minDistanceVolume = volume;
                    targetDistance = closestDistanceMin;
                }
            }

            if (globalVolume != null && minDistanceVolume == null)
            {
                minDistanceVolume = globalVolume;
            }
            
            onOverrideFinish?.Invoke(stack);

            // URPTimelineVolumnLight.instance.Effect(camera);


            // #if UNITY_EDITOR && !REAL_PERFORMANCE_MODE
            // #if UNITY_EDITOR
            //             if (!Application.isPlaying || isInSkillEditor)
            {
                foreach (var component in stack.components.Values)
                {
                    if (!component.active)
            ...
            CheckDistanceVolume(camera, minDistanceVolume, mainCamera);
            //             else
            // #endif
            //             {
            //                 if(camera != null)VolumeStrategicUpadte(camera);
            //             }

        }

        private Volume lastVolume = null, callbackVolume = null;
        /// <summary>

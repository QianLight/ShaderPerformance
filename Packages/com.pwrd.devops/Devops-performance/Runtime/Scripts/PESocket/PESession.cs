...
namespace PENet {
    public abstract class PESession : PEMsg {
        private Socket skt;
        ...
        private void RcvHeadData(IAsyncResult ar) {
            ...
            try {
                ...
                PETool.LogMsg("RcvHeadError:" + e.StackTrace, LogLevel.Error);
            }
        }
            ...
            try {
                ...
                if (len > 0) {
                    ...
                    if (pack.bodyIndex < pack.bodyLen) {
                        ...
                        OnReciveMsg(/*pack.msgId*/0, pack.bodyBuff);
        ...
        protected virtual void OnReciveMsg(int msgId, byte[] buffer)
        {
